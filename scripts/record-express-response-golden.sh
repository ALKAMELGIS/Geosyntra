#!/usr/bin/env bash
# Record Express responses into migration/express-response-golden.tsv (public routes only).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT="$ROOT/migration/express-response-golden.tsv"
...
{
  echo "# Express public-route response golden file (Task 18)."
  echo "# Columns: METHOD, PATH, BODY_JSON (or empty), STATUS, EXPECT"
  echo "# Regenerate: EXPRESS_URL=... scripts/record-express-response-golden.sh"
  record GET /health
  ...
  record GET /api/geo/grounding/status
  record GET /api/geo/locations
  record GET /api/github/status
  record GET /api/github/events
  record GET /api/weather/latest
  record POST /api/ai/analyze '{}'
