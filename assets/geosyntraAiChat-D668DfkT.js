import{g as d}from"./geoExplorerGeminiApi-egIRxMiY.js";const b=`You are Geosyntra AI — a professional assistant for agriculture, GIS-backed farm data, and clear explanations.
...
- **Reply language:** Follow the "UI locale — reply language" line appended immediately after this system block (English or Arabic per user app settings).`;function g(s,r){const e=s.map(t=>({role:t.role==="assistant"?"model":"user",parts:[{text:t.text}]}));return e.push({role:"user",parts:[{text:r}]}),e}async function I(s){const{apiKey:r,systemInstruction:e,turns:t,userMessage:o}=s;return d({apiKey:r,systemInstruction:e,contents:g(t,o)})}const f="deepseek-chat";async function S(s){var h,c,u,p,y;const{apiKey:r,system:e,turns:t,userMessage:o}=s,i=[{role:"system",content:e}];for(const m of t)i.push({role:m.role==="user"?"user":"assistant",content:m.text});i.push({role:"user",content:o});const a=await fetch("https://api.deepseek.com/v1/chat/completions",{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${r}`},body:JSON.stringify({model:f,messages:i,max_tokens:4096})}),n=await a.json().catch(()=>({}));if(!a.ok)throw new Error(((h=n==null?void 0:n.error)==null?void 0:h.message)||a.statusText||`HTTP ${a.status}`);const l=(y=(p=(u=(c=n.choices)==null?void 0:c[0])==null?void 0:u.message)==null?void 0:p.content)==null?void 0:y.trim();if(!l)throw new Error("Empty DeepSeek response");return l}export{b as GEOSYNTRA_AI_CHAT_SYSTEM,S as geosyntraChatWithDeepSeek,I as geosyntraChatWithGemini};
