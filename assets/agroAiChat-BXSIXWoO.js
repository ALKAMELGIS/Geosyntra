import{r as g,ag as f,ah as d}from"./index-BG_6TgJe.js";import{g as w}from"./geoExplorerGemini-D2wVs9Ez.js";function G(){return g.useSyncExternalStore(f,d,d)}const x=`You are AgriCloud AI Agro-Chat — a professional assistant for agriculture, GIS-backed farm data, and clear explanations.
...
- **Reply language:** Follow the "UI locale — reply language" line appended immediately after this system block (English or Arabic per user app settings).`;function b(s,r){const e=s.map(t=>({role:t.role==="assistant"?"model":"user",parts:[{text:t.text}]}));return e.push({role:"user",parts:[{text:r}]}),e}async function A(s){const{apiKey:r,systemInstruction:e,turns:t,userMessage:n}=s;return w({apiKey:r,systemInstruction:e,contents:b(t,n)})}const I="deepseek-chat";async function E(s){var u,h,c,p,y;const{apiKey:r,system:e,turns:t,userMessage:n}=s,i=[{role:"system",content:e}];for(const m of t)i.push({role:m.role==="user"?"user":"assistant",content:m.text});i.push({role:"user",content:n});const a=await fetch("https://api.deepseek.com/v1/chat/completions",{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${r}`},body:JSON.stringify({model:I,messages:i,max_tokens:4096})}),o=await a.json().catch(()=>({}));if(!a.ok)throw new Error(((u=o==null?void 0:o.error)==null?void 0:u.message)||a.statusText||`HTTP ${a.status}`);const l=(y=(p=(c=(h=o.choices)==null?void 0:h[0])==null?void 0:c.message)==null?void 0:p.content)==null?void 0:y.trim();if(!l)throw new Error("Empty DeepSeek response");return l}export{x as A,E as a,A as b,G as u};
