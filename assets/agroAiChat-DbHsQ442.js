import{r as d,ak as f,al as g}from"./index-6YQJDe0N.js";import{g as w}from"./geoExplorerGemini-DasrU6An.js";function x(){return d.useSyncExternalStore(f,g,g)}const G=`You are AgriCloud AI Agro-Chat — a professional assistant for agriculture, GIS-backed farm data, and clear explanations.
...
- Reply in the **same language** the user wrote in (Arabic or English) unless they explicitly mix languages.`;function b(s,r){const e=s.map(t=>({role:t.role==="assistant"?"model":"user",parts:[{text:t.text}]}));return e.push({role:"user",parts:[{text:r}]}),e}async function A(s){const{apiKey:r,systemInstruction:e,turns:t,userMessage:n}=s;return w({apiKey:r,systemInstruction:e,contents:b(t,n)})}const I="deepseek-chat";async function E(s){var u,h,c,p,y;const{apiKey:r,system:e,turns:t,userMessage:n}=s,i=[{role:"system",content:e}];for(const m of t)i.push({role:m.role==="user"?"user":"assistant",content:m.text});i.push({role:"user",content:n});const a=await fetch("https://api.deepseek.com/v1/chat/completions",{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${r}`},body:JSON.stringify({model:I,messages:i,max_tokens:4096})}),o=await a.json().catch(()=>({}));if(!a.ok)throw new Error(((u=o==null?void 0:o.error)==null?void 0:u.message)||a.statusText||`HTTP ${a.status}`);const l=(y=(p=(c=(h=o.choices)==null?void 0:h[0])==null?void 0:c.message)==null?void 0:p.content)==null?void 0:y.trim();if(!l)throw new Error("Empty DeepSeek response");return l}export{G as A,E as a,A as b,x as u};
