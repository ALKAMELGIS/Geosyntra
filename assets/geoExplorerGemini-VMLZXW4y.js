import{r as w,ai as S,aj as p}from"./index-CX7g-5FY.js";function k(){return w.useSyncExternalStore(S,p,p)}const L=`You are "Geo Explorer" / Geo AI: a concise assistant inside a map workspace (satellite globe or GIS map).
...
Do not put MAP_QUERY inside markdown code fences.`,C=`### GEO AI COPILOT (mission — integrate GIS + map + weather)
...
**8. Fail-safe** — No fabricated coords or figures; when anchors+facts insufficient for spatial confidence, ask for clarification per §1.`,G=`LAYER DATA rules (when "LAYER DATA" / layer list / GIS Content sections appear):
...
- **General questions:** If there is no layer tie, answer from general knowledge; MAP_QUERY only when a single global place is clearly intended.`,U=`Session continuity & weather (read carefully when the next blocks appear):
...
- Conversations are sequential: short follow-ups (“coordinates of that place”, “same feature”, “what country”, “أعطني الإحداثيات”, “نفس الموقع”) refer to the last matched feature or SESSION MAP ANCHOR unless the user names a new layer or ID.`;function f(t,e){return Number.isFinite(t)&&Number.isFinite(e)&&t>=-180&&t<=180&&e>=-90&&e<=90}function y(t){const e=t.match(/MAP_QUERY:\s*([-\d.]+)\s*,\s*([-\d.]+)/i);if(!e)return null;const a=Number(e[1]),n=Number(e[2]);return!Number.isFinite(a)||!Number.isFinite(n)?null:f(a,n)?[a,n]:f(n,a)?[n,a]:null}function O(t){return t.parts.filter(e=>e.type==="text").map(e=>e.text).join(`
`)}function R(t){return t.replace(/\r?\nMAP_QUERY:\s*[^\n]+/gi,"").replace(/^MAP_QUERY:\s*[^\n]+\r?\n?/i,"").trimEnd()}function I(t){let e=t.trimEnd();return e=e.replace(/\n\n\(Map centered on the best place-name match for your message\.\)/gi,""),e=e.replace(/\n\n\(Map pin from layer[\s\S]*$/m,""),e=e.replace(/\n\n\*\*Map:\*\*[\s\S]*$/,""),e.trimEnd()}function v(t){const e=t.trimEnd(),n=e.lastIndexOf("GEO_AI_JSON:");if(n<0)return t;const r=e.lastIndexOf(`
`,n);return(r>=0?e.slice(0,r):e.slice(0,n)).trimEnd()}function Y(t){return v(I(R(t))).replace(/\*/g,"").trimEnd()}function N(t){return t.map(e=>e.type==="text"?{text:e.text}:{inline_data:{mime_type:e.mime,data:e.base64}})}function D(t){return t.map(e=>({role:e.role,parts:N(e.parts)}))}function Q(t){for(let e=t.length-1;e>=0;e--){const a=t[e];if(a.role!=="model")continue;const n=y(O(a));if(n)return n}return null}function F(t){for(let e=t.length-1;e>=0;e--){const a=t[e];if(a.role!=="assistant"&&a.role!=="model")continue;const n=y(a.text);if(n)return n}return null}const _=["gemini-2.5-flash","gemini-2.5-flash-lite","gemini-flash-latest","gemini-2.5-pro","gemini-1.5-flash","gemini-1.5-flash-8b","gemini-1.5-pro"],T=["v1beta","v1"];function P(t){const e=t.toLowerCase();return e.includes("api key not valid")||e.includes("invalid api key")||e.includes("invalid argument")&&e.includes("key")}function M(t,e){const a=e.toLowerCase();return t===404||t===400||t===403||t===429||t===503||a.includes("quota")||a.includes("exceeded")||a.includes("billing")||a.includes("limit: 0")||a.includes("resource_exhausted")||a.includes("resource exhausted")||a.includes("rate limit")||a.includes("rate_limit")||a.includes("overloaded")||a.includes("not found")||a.includes("is not found")||a.includes("not supported")||a.includes("permission_denied")||a.includes("permission denied")}async function W(t){var l,c,u,d,h;const{apiKey:e,systemInstruction:a,contents:n}=t;let r="Unknown error";for(const E of _)for(const g of T){const b=`https://generativelanguage.googleapis.com/${g}/models/${E}:generateContent?key=${encodeURIComponent(e)}`,o=await fetch(b,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({systemInstruction:{parts:[{text:a}]},contents:n})}),s=await o.json().catch(()=>({}));if(!o.ok){if(r=((l=s==null?void 0:s.error)==null?void 0:l.message)||o.statusText||`HTTP ${o.status}`,P(String(r)))throw new Error(r);if(M(o.status,String(r)))continue;throw new Error(r)}const m=((h=(d=(u=(c=s==null?void 0:s.candidates)==null?void 0:c[0])==null?void 0:u.content)==null?void 0:d.parts)==null?void 0:h.map(A=>A.text).filter(Boolean).join(""))??"";if(!m){r="Empty model response";continue}return m}const i=/quota|exceeded|rate|billing|limit:\s*0/i.test(r)?" Enable billing in Google AI Studio / Cloud console, or wait and retry; free-tier limits vary by model.":"";throw new Error(`${r}${i}`)}export{C as G,F as a,U as b,D as c,R as d,L as e,G as f,W as g,Q as l,O as m,y as p,Y as s,k as u};
