import{buildGisContentLayersContext as X}from"./geoAiChatClaude-BEQWbI01.js";import{Z as j,b8 as V,d as Z,aa as z,b9 as J,i as ee,e as te,G as ae,p as O,ba as c,bb as h,bc as b,h as ne,bd as P,be as oe,bf as ie,bg as se,bh as re,bi as le,bj as ce,bk as de,bl as me,bm as pe,ad as ue,bn as ye,ae as fe}from"./fieldsStore-CyTAQMdQ.js";import{a2 as Re}from"./fieldsStore-CyTAQMdQ.js";import{g as he}from"./geoExplorerGeminiApi-egIRxMiY.js";import"./index-BUEY8dQB.js";function ge(p,u){const e=(u[1]-p[1])*Math.PI/180,d=(u[0]-p[0])*Math.PI/180,g=p[1]*Math.PI/180,$=u[1]*Math.PI/180,M=Math.sin(e/2)*Math.sin(e/2)+Math.cos(g)*Math.cos($)*Math.sin(d/2)*Math.sin(d/2);return 2*6371*Math.asin(Math.min(1,Math.sqrt(M)))}async function Ie(p){var T;const{apiKey:u,historyWithUser:A,userTextForMapFallback:e,primaryVectorLayers:d,mapboxAccessToken:g,openWeatherApiKey:$,pinLngLat:M,lastMapQueryCoords:C,inspectAnchorLngLat:N,mapPopup:Q,addedLayersHeading:U,attachGisSavedLayers:k,extraSystemAppend:E,questionEditInPlace:D}=p,F=k===!0;let m=[...d],R=`### GIS Content (saved layers from GIS Map)
(Not attached on this page — open **GIS Map** → Geo AI to use layers saved in IndexedDB.)`;if(F){const i=await j();m=[...d,...i.map(n=>({name:n.name,visible:n.visible,source:n.source,data:n.data,arcgisLayerDefinition:n.arcgisLayerDefinition}))],R=await X(22e3)}const q=V(d,2e4),l=e.trim().length>0?Z(e,m):null;let _="";l&&l.score>=32&&(_=`
...
- Attributes:
${l.matchSummary}`);const B=await z({userText:e,pinLngLat:M,lastMapQueryCoords:C,inspectAnchorLngLat:N,combinedLayers:m,mapboxAccessToken:g,openWeatherApiKey:$,mapPopup:Q}),H=E!=null&&E.trim()?`
...
The user edited their latest question in the same thread (no new chat). Prior assistant replies after that question are not in this history. Answer only the **updated** wording: apply new field/layer/selection/stat instructions concisely. Skip greetings, recap, and generic onboarding.`:""}${H}`;let t=await he({apiKey:u,systemInstruction:W,contents:J(A)});const G=ee(e,m),Y=((T=e?te(e,m):null)==null?void 0:T.trim())??"",S=l,a=S&&S.score>=ae?S:null;let s=O(t);if(G&&!a&&s&&(t=`${c(t).trimEnd()}
