import{e as d,c as g}from"./geoExplorerGeminiApi-CaWaKKit.js";import{h as f}from"./index-BYvECsyR.js";const G=`You are Geosyntra AI — a professional assistant for agriculture, GIS-backed farm data, and clear explanations.
...
- **Reply language:** Follow the "UI locale — reply language" line appended immediately after this system block (English or Arabic per user app settings).`;function w(r,e){const t=r.map(s=>({role:s.role==="assistant"?"model":"user",parts:[{text:s.text}]}));return t.push({role:"user",parts:[{text:e}]}),t}async function k(r){const{apiKey:e,systemInstruction:t,turns:s,userMessage:a}=r;return g({...e!=null&&e.trim()&&e!=="__gateway__"?{apiKey:e}:{},systemInstruction:t,contents:w(s,a)})}const b="deepseek-chat";async function x(r){var h,c,u,p,y;const{apiKey:e,system:t,turns:s,userMessage:a}=r;if(f()||e==="__gateway__")return d({system:t,turns:s,userMessage:a});const i=[{role:"system",content:t}];for(const m of s)i.push({role:m.role==="user"?"user":"assistant",content:m.text});i.push({role:"user",content:a});const o=await fetch("https://api.deepseek.com/v1/chat/completions",{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${e}`},body:JSON.stringify({model:b,messages:i,max_tokens:4096})}),n=await o.json().catch(()=>({}));if(!o.ok)throw new Error(((h=n==null?void 0:n.error)==null?void 0:h.message)||o.statusText||`HTTP ${o.status}`);const l=(y=(p=(u=(c=n.choices)==null?void 0:c[0])==null?void 0:u.message)==null?void 0:p.content)==null?void 0:y.trim();if(!l)throw new Error("Empty DeepSeek response");return l}export{G as GEOSYNTRA_AI_CHAT_SYSTEM,x as geosyntraChatWithDeepSeek,k as geosyntraChatWithGemini};
