import{av as b,Z as v,aw as C,ax as w,w as O}from"./index-8hoU7XkU.js";import{e as Q,s as U,h as B,i as D}from"./GeoExplorerGeminiChatBody-BMFZGNux.js";import{g as k,a as F,p as $,s as H,G as N,b as W,c as q}from"./geoExplorerGemini-CYVbl3KW.js";function K(n,l){const a=(l[1]-n[1])*Math.PI/180,i=(l[0]-n[0])*Math.PI/180,p=n[1]*Math.PI/180,y=l[1]*Math.PI/180,u=Math.sin(a/2)*Math.sin(a/2)+Math.cos(p)*Math.cos(y)*Math.sin(i/2)*Math.sin(i/2);return 2*6371*Math.asin(Math.min(1,Math.sqrt(u)))}async function j(n){var G;const{apiKey:l,historyWithUser:h,userTextForMapFallback:a,primaryVectorLayers:i,mapboxAccessToken:p,openWeatherApiKey:y,pinLngLat:u,lastMapQueryCoords:x,mapPopup:I,addedLayersHeading:S,attachGisSavedLayers:P}=n,R=P===!0;let f=[...i],L=`### GIS Content (saved layers from GIS Map)
...
${N}`;g+=await Q({userText:a,pinLngLat:u,lastMapQueryCoords:x,combinedLayers:f,mapboxAccessToken:p,openWeatherApiKey:y,mapPopup:I});const T=`${W}
...
${L}`,c=await k({apiKey:l,systemInstruction:T,contents:F(h)}),d=$(c),A=((G=a?w(a):null)==null?void 0:G.trim())??"",e=a.trim().length>0?O(a,f):null;let o=null,r=c,m="geocode";const M=s=>{const t=s.matchSummary.trim();return`

(Map pin from layer "${s.layerName}" — matched feature attributes: ${t.slice(0,200)}${t.length>200?"…":""})`};if(!!e&&(!!A||!!(d&&e&&K(d,[e.lng,e.lat])>2&&e.score>=22))&&e)o=[e.lng,e.lat],m="layer",r=`${H(c).trimEnd()}${M(e)}`;else if(d)o=d,m="map_query",r=c;else if(a)if(e)o=[e.lng,e.lat],m="layer",r=`${c.trimEnd()}${M(e)}`;else{const s=U(B(a));if(s.length>=2){const t=await D(s,{mapboxAccessToken:p});t&&(o=t,m="geocode",r=`${c.trimEnd()}
