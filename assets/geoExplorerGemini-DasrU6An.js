...
- Conversations are sequential: short follow-ups (“coordinates of that place”, “same feature”, “what country”, “أعطني الإحداثيات”, “نفس الموقع”) refer to the last matched feature or SESSION MAP ANCHOR unless the user names a new layer or ID.`;function f(t,e){return Number.isFinite(t)&&Number.isFinite(e)&&t>=-180&&t<=180&&e>=-90&&e<=90}function y(t){const e=t.match(/MAP_QUERY:\s*([-\d.]+)\s*,\s*([-\d.]+)/i);if(!e)return null;const a=Number(e[1]),n=Number(e[2]);return!Number.isFinite(a)||!Number.isFinite(n)?null:f(a,n)?[a,n]:f(n,a)?[n,a]:null}function w(t){return t.parts.filter(e=>e.type==="text").map(e=>e.text).join(`
`)}function S(t){return t.replace(/\r?\nMAP_QUERY:\s*[^\n]+/gi,"").replace(/^MAP_QUERY:\s*[^\n]+\r?\n?/i,"").trimEnd()}function O(t){let e=t.trimEnd();return e=e.replace(/\n\n\(Map centered on the best place-name match for your message\.\)/gi,""),e=e.replace(/\n\n\(Map pin from layer[\s\S]*$/m,""),e=e.replace(/\n\n\*\*Map:\*\*[\s\S]*$/,""),e.trimEnd()}function R(t){const e=t.trimEnd(),n=e.lastIndexOf("GEO_AI_JSON:");if(n<0)return t;const r=e.lastIndexOf(`
`,n);return(r>=0?e.slice(0,r):e.slice(0,n)).trimEnd()}function C(t){return R(O(S(t))).replace(/\*/g,"").trimEnd()}function I(t){return t.map(e=>e.type==="text"?{text:e.text}:{inline_data:{mime_type:e.mime,data:e.base64}})}function G(t){return t.map(e=>({role:e.role,parts:I(e.parts)}))}function U(t){for(let e=t.length-1;e>=0;e--){const a=t[e];if(a.role!=="model")continue;const n=y(w(a));if(n)return n}return null}function Y(t){for(let e=t.length-1;e>=0;e--){const a=t[e];if(a.role!=="assistant"&&a.role!=="model")continue;const n=y(a.text);if(n)return n}return null}const v=["gemini-2.5-flash","gemini-2.5-flash-lite","gemini-flash-latest","gemini-2.5-pro","gemini-1.5-flash","gemini-1.5-flash-8b","gemini-1.5-pro"],N=["v1beta","v1"];function _(t,e){const a=`System (follow strictly):
${t}

---

`,n=e.map(s=>({role:s.role,parts:s.parts.map(c=>({...c}))})),r=n.findIndex(s=>s.role==="user");if(r<0)return[{role:"user",parts:[{text:a.trimEnd()}]},...n];const o=[...n[r].parts];if(o.length===0)o.push({text:a.trimEnd()});else{const s=o[0];typeof(s==null?void 0:s.text)=="string"?o[0]={text:a+s.text}:o.unshift({text:a.trimEnd()})}return n[r]={role:"user",parts:o},n}function T(t){const e=t.toLowerCase();return e.includes("api key not valid")||e.includes("invalid api key")||e.includes("invalid argument")&&e.includes("key")}function P(t,e){const a=e.toLowerCase();return t===404||t===400||t===403||t===429||t===503||a.includes("quota")||a.includes("exceeded")||a.includes("billing")||a.includes("limit: 0")||a.includes("resource_exhausted")||a.includes("resource exhausted")||a.includes("rate limit")||a.includes("rate_limit")||a.includes("overloaded")||a.includes("not found")||a.includes("is not found")||a.includes("not supported")||a.includes("permission_denied")||a.includes("permission denied")}async function D(t){var s,c,u,d,h;const{apiKey:e,systemInstruction:a,contents:n}=t;let r="Unknown error";for(const E of v)for(const m of N){const g=`https://generativelanguage.googleapis.com/${m}/models/${E}:generateContent?key=${encodeURIComponent(e)}`,b=m==="v1beta"?{systemInstruction:{parts:[{text:a}]},contents:n}:{contents:_(a,n)},l=await fetch(g,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(b)}),i=await l.json().catch(()=>({}));if(!l.ok){if(r=((s=i==null?void 0:i.error)==null?void 0:s.message)||l.statusText||`HTTP ${l.status}`,T(String(r)))throw new Error(r);if(P(l.status,String(r)))continue;throw new Error(r)}const p=((h=(d=(u=(c=i==null?void 0:i.candidates)==null?void 0:c[0])==null?void 0:u.content)==null?void 0:d.parts)==null?void 0:h.map(A=>A.text).filter(Boolean).join(""))??"";if(!p){r="Empty model response";continue}return p}const o=/quota|exceeded|rate|billing|limit:\s*0/i.test(r)?" Enable billing in Google AI Studio / Cloud console, or wait and retry; free-tier limits vary by model.":"";throw new Error(`${r}${o}`)}export{x as G,Y as a,L as b,G as c,S as d,M as e,k as f,D as g,U as l,w as m,y as p,C as s};
