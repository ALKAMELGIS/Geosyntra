...
- Conversations are sequential: short follow-ups (“coordinates of that place”, “same feature”, “what country”, “أعطني الإحداثيات”, “نفس الموقع”) refer to the last matched feature or SESSION MAP ANCHOR unless the user names a new layer or ID.`;function f(n,e){return Number.isFinite(n)&&Number.isFinite(e)&&n>=-180&&n<=180&&e>=-90&&e<=90}function y(n){const e=n.match(/MAP_QUERY:\s*([-\d.]+)\s*,\s*([-\d.]+)/i);if(!e)return null;const t=Number(e[1]),a=Number(e[2]);return!Number.isFinite(t)||!Number.isFinite(a)?null:f(t,a)?[t,a]:f(a,t)?[a,t]:null}function A(n){const e=[];for(const t of n.parts)if(t.type==="text")e.push(t.text);else if(t.type==="dataTable"){const a=t.table;e.push(`[Table: ${a.title??a.kind} (${a.rows.length} rows)]`)}return e.join(`
`)}function S(n){return n.replace(/\r?\nMAP_QUERY:\s*[^\n]+/gi,"").replace(/^MAP_QUERY:\s*[^\n]+\r?\n?/i,"").trimEnd()}function O(n){let e=n.trimEnd();return e=e.replace(/\n\n\(Map centered on the best place-name match for your message\.\)/gi,""),e=e.replace(/\n\n\(Map centered on "[^"]*" — geocoder confidence OK\.\)/gi,""),e=e.replace(/\n\n\(Map pin from layer[\s\S]*$/m,""),e=e.replace(/\n\n\*\*Map:\*\*[\s\S]*$/,""),e.trimEnd()}function R(n){const e=n.trimEnd(),a=e.lastIndexOf("GEO_AI_JSON:");if(a<0)return n;const s=e.lastIndexOf(`
`,a);return(s>=0?e.slice(0,s):e.slice(0,a)).trimEnd()}function C(n){return R(O(S(n))).replace(/\*\*([^*]+)\*\*/g,"$1").replace(/\*/g,"").trimEnd()}function I(n){const e=[];for(const t of n)if(t.type==="text")e.push({text:t.text});else if(t.type==="image")e.push({inline_data:{mime_type:t.mime,data:t.base64}});else{const a=t.table,s=a.columns.map(i=>i.label).join(" | "),r=a.rows.slice(0,12).map(i=>a.columns.map(u=>String(i.values[u.key]??"")).join(" | ")).join(`
`),o=`[Geo AI structured table omitted from vision — ${a.kind}: ${a.rows.length} rows. Columns: ${s}${r?`
Sample:
${r}`:""}]`;e.push({text:o})}return e}function G(n){return n.map(e=>({role:e.role,parts:I(e.parts)}))}function U(n){for(let e=n.length-1;e>=0;e--){const t=n[e];if(t.role!=="model")continue;const a=y(A(t));if(a)return a}return null}function Y(n){for(let e=n.length-1;e>=0;e--){const t=n[e];if(t.role!=="assistant"&&t.role!=="model")continue;const a=y(t.text);if(a)return a}return null}const v=["gemini-2.5-flash","gemini-2.5-flash-lite","gemini-flash-latest","gemini-2.5-pro","gemini-1.5-flash","gemini-1.5-flash-8b","gemini-1.5-pro"],N=["v1beta","v1"];function _(n,e){const t=`System (follow strictly):
${n}

---

`,a=e.map(o=>({role:o.role,parts:o.parts.map(i=>({...i}))})),s=a.findIndex(o=>o.role==="user");if(s<0)return[{role:"user",parts:[{text:t.trimEnd()}]},...a];const r=[...a[s].parts];if(r.length===0)r.push({text:t.trimEnd()});else{const o=r[0];typeof(o==null?void 0:o.text)=="string"?r[0]={text:t+o.text}:r.unshift({text:t.trimEnd()})}return a[s]={role:"user",parts:r},a}function T(n){const e=n.toLowerCase();return e.includes("api key not valid")||e.includes("invalid api key")||e.includes("invalid argument")&&e.includes("key")}function M(n,e){const t=e.toLowerCase();return n===404||n===400||n===403||n===429||n===503||t.includes("quota")||t.includes("exceeded")||t.includes("billing")||t.includes("limit: 0")||t.includes("resource_exhausted")||t.includes("resource exhausted")||t.includes("rate limit")||t.includes("rate_limit")||t.includes("overloaded")||t.includes("not found")||t.includes("is not found")||t.includes("not supported")||t.includes("permission_denied")||t.includes("permission denied")}async function D(n){var o,i,u,d,h;const{apiKey:e,systemInstruction:t,contents:a}=n;let s="Unknown error";for(const E of v)for(const m of N){const g=`https://generativelanguage.googleapis.com/${m}/models/${E}:generateContent?key=${encodeURIComponent(e)}`,b=m==="v1beta"?{systemInstruction:{parts:[{text:t}]},contents:a}:{contents:_(t,a)},c=await fetch(g,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(b)}),l=await c.json().catch(()=>({}));if(!c.ok){if(s=((o=l==null?void 0:l.error)==null?void 0:o.message)||c.statusText||`HTTP ${c.status}`,T(String(s)))throw new Error(s);if(M(c.status,String(s)))continue;throw new Error(s)}const p=((h=(d=(u=(i=l==null?void 0:l.candidates)==null?void 0:i[0])==null?void 0:u.content)==null?void 0:d.parts)==null?void 0:h.map(w=>w.text).filter(Boolean).join(""))??"";if(!p){s="Empty model response";continue}return p}const r=/quota|exceeded|rate|billing|limit:\s*0/i.test(s)?" Enable billing in Google AI Studio / Cloud console, or wait and retry; free-tier limits vary by model.":"";throw new Error(`${s}${r}`)}export{x as G,Y as a,L as b,S as c,P as d,k as e,D as g,U as l,G as m,y as p,C as s};
