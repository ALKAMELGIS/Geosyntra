import{buildGisContentLayersContext as oe}from"./geoAiChatClaude-C5AFVBfu.js";import{k as ae,B as ie,y as se,q as Q,z as le,C as re,D as ce,E as de,F as ue,G as pe,H as ge,I as me,J as fe,g as V,e as x,K as A,L as k,M as he,N as B,O as ye,P as Le,Q as $e,R as Ee,S as Ge,t as xe,x as Me,T as be,U as Se,V as ve,W as Pe,X as we,Y as _e}from"./SatelliteIntelligenceMain-BJEFg3sx.js";import{Z as at}from"./SatelliteIntelligenceMain-BJEFg3sx.js";import{c as Ie}from"./geoExplorerGeminiApi-BJ4YURk8.js";import{d as Ae}from"./intentDetector-Cg7MVrOW.js";import{readGeoSpatialMemory as Re,groundingSuggestionChipsFromMemory as Ce,writeGeoSpatialMemory as Oe}from"./spatialMemory-DwP1Bk4f.js";import"./index-BLNEbX-K.js";import"./useSubscription-D02A8iDU.js";import"./sentinelHubWmsCapabilities-DTxTrAg3.js";import"./systemTokensApi-CbIhRLlS.js";import"./index-BTnpYR6i.js";function Te(e){if(!e)return"";const t=["### Geo Dataset Engine — AOI snapshot"];return e.label&&t.push(`- Label: ${e.label}`),e.bbox&&t.push(`- Bbox (W,S,E,N): ${e.bbox.map(h=>Number(h).toFixed(5)).join(", ")}`),e.areaHa!=null&&t.push(`- Area (ha): ${e.areaHa.toFixed(2)}`),e.timelineLabel&&t.push(`- Timeline: ${e.timelineLabel}`),e.layerId&&t.push(`- Layer: ${e.layerId}`),e.ndviMean!=null&&t.push(`- NDVI: mean ${e.ndviMean.toFixed(3)}${e.ndviMin!=null?`, min ${e.ndviMin.toFixed(3)}`:""}${e.ndviMax!=null?`, max ${e.ndviMax.toFixed(3)}`:""}`),t.length>1?t.join(`
...
The user edited their latest question in the same thread (no new chat). Prior assistant replies after that question are not in this history. Answer only the **updated** wording: apply new field/layer/selection/stat instructions concisely. Skip greetings, recap, and generic onboarding.`:""}${Z}`;let i=await Ie({...t!=null&&t.trim()&&t!=="__gateway__"?{apiKey:t}:{},systemInstruction:ee,contents:pe(h)});const F=ge(n,L),te=((j=n?me(n,L):null)==null?void 0:j.trim())??"",U=S,r=U&&U.score>=fe?U:null;let E=V(i);if(F&&!r&&E&&(i=`${x(i).trimEnd()}

**Map:** MAP_QUERY was removed — no confident feature match in your active or GIS Content layers; the app will not move the map to an unrelated location.${A(k(n))}`,E=null),E&&!r){const l=he({userText:n,replyText:i,mapQueryCoords:E,strongLayerHit:r});if(!l.allow){const p=k(n);i=`${x(i).trimEnd()}`,i=B(i,p,"lowConfidenceMapQuery",l.confidence),i=`${i.trimEnd()}${A(p)}`,E=null}}let $=null,u=i,w="geocode";const H=l=>{const p=l.matchSummary.trim();return`

(Map pin from layer "${l.layerName}" — matched feature attributes: ${p.slice(0,200)}${p.length>200?"…":""})`};if(!!r&&(!!te||!!(E&&r&&Qe(E,[r.lng,r.lat])>2))&&r)$=[r.lng,r.lat],w="layer",u=`${x(i).trimEnd()}${H(r)}`;else if(E&&(!F||r))$=E,w="map_query",u=i;else if(n){if(r)$=[r.lng,r.lat],w="layer",u=`${i.trimEnd()}${H(r)}`;else if(ye(n,L)&&!(Le(n)&&!$e(n))){const l=Ee(Ge(n));if(l.length>=2){const p=await xe(l,{mapboxAccessToken:a}),{chosen:N,ambiguous:K}=Me(p),I=k(n);if(N&&!K){$=[N.lng,N.lat],w="geocode";const D=N.label.replace(/\s+/g," ").trim().slice(0,160);u=`${x(i).trimEnd()}

(Map centered on "${D}" — geocoder confidence OK.)`}else if(p.length>=2&&K){const D=p.slice(0,3).map(ne=>ne.label.split(",").slice(0,2).join(",").trim()).filter(Boolean);u=be(x(i).trimEnd(),I,D)}else p.length?u=`${B(x(i).trimEnd(),I,"insufficientData").trimEnd()}${A(I)}`:u=`${B(x(i).trimEnd(),I,"cannotLocatePrecisely").trimEnd()}${A(I)}`}}else if(f&&!F)$=f,w="grounding",u=`${x(i).trimEnd()}
