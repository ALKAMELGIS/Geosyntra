...
`)}function S(a){return a.replace(/\r?\nMAP_QUERY:\s*[^\n]+/gi,"").replace(/^MAP_QUERY:\s*[^\n]+\r?\n?/i,"").trimEnd()}function R(a){let e=a.trimEnd();return e=e.replace(/\n\n\(Map centered on the best place-name match for your message\.\)/gi,""),e=e.replace(/\n\n\(Map centered on "[^"]*" — geocoder confidence OK\.\)/gi,""),e=e.replace(/\n\n\(Map pin from layer[\s\S]*$/m,""),e=e.replace(/\n\n\*\*Map:\*\*[\s\S]*$/,""),e.trimEnd()}function O(a){const e=a.trimEnd(),n=e.lastIndexOf("GEO_AI_JSON:");if(n<0)return a;const s=e.lastIndexOf(`
`,n);return(s>=0?e.slice(0,s):e.slice(0,n)).trimEnd()}function G(a){return O(R(S(a))).replace(/\*\*([^*]+)\*\*/g,"$1").replace(/\*/g,"").trimEnd()}function I(a){const e=[];for(const t of a)if(t.type==="text")e.push({text:t.text});else if(t.type==="image")e.push({inline_data:{mime_type:t.mime,data:t.base64}});else{const n=t.table,s=n.columns.map(i=>i.label).join(" | "),r=n.rows.slice(0,12).map(i=>n.columns.map(u=>String(i.values[u.key]??"")).join(" | ")).join(`
`),o=`[Geo AI structured table omitted from vision — ${n.kind}: ${n.rows.length} rows. Columns: ${s}${r?`
Sample:
${r}`:""}]`;e.push({text:o})}return e}function U(a){return a.map(e=>({role:e.role,parts:I(e.parts)}))}function Y(a){for(let e=a.length-1;e>=0;e--){const t=a[e];if(t.role!=="model")continue;const n=y(A(t));if(n)return n}return null}function D(a){for(let e=a.length-1;e>=0;e--){const t=a[e];if(t.role!=="assistant"&&t.role!=="model")continue;const n=y(t.text);if(n)return n}return null}const v=["gemini-2.5-flash","gemini-2.5-flash-lite","gemini-flash-latest","gemini-2.5-pro","gemini-1.5-flash","gemini-1.5-flash-8b","gemini-1.5-pro"],N=["v1beta","v1"];function T(a,e){const t=`System (follow strictly):
${a}

---

`,n=e.map(o=>({role:o.role,parts:o.parts.map(i=>({...i}))})),s=n.findIndex(o=>o.role==="user");if(s<0)return[{role:"user",parts:[{text:t.trimEnd()}]},...n];const r=[...n[s].parts];if(r.length===0)r.push({text:t.trimEnd()});else{const o=r[0];typeof(o==null?void 0:o.text)=="string"?r[0]={text:t+o.text}:r.unshift({text:t.trimEnd()})}return n[s]={role:"user",parts:r},n}function _(a){const e=a.toLowerCase();return e.includes("api key not valid")||e.includes("invalid api key")||e.includes("invalid argument")&&e.includes("key")}function x(a,e){const t=e.toLowerCase();return a===404||a===400||a===403||a===429||a===503||t.includes("quota")||t.includes("exceeded")||t.includes("billing")||t.includes("limit: 0")||t.includes("resource_exhausted")||t.includes("resource exhausted")||t.includes("rate limit")||t.includes("rate_limit")||t.includes("overloaded")||t.includes("not found")||t.includes("is not found")||t.includes("not supported")||t.includes("permission_denied")||t.includes("permission denied")}async function Q(a){var o,i,u,d,p;const{apiKey:e,systemInstruction:t,contents:n}=a;let s="Unknown error";for(const E of v)for(const h of N){const g=`https://generativelanguage.googleapis.com/${h}/models/${E}:generateContent?key=${encodeURIComponent(e)}`,b=h==="v1beta"?{systemInstruction:{parts:[{text:t}]},contents:n}:{contents:T(t,n)},c=await fetch(g,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(b)}),l=await c.json().catch(()=>({}));if(!c.ok){if(s=((o=l==null?void 0:l.error)==null?void 0:o.message)||c.statusText||`HTTP ${c.status}`,_(String(s)))throw new Error(s);if(x(c.status,String(s)))continue;throw new Error(s)}const m=((p=(d=(u=(i=l==null?void 0:l.candidates)==null?void 0:i[0])==null?void 0:u.content)==null?void 0:d.parts)==null?void 0:p.map(w=>w.text).filter(Boolean).join(""))??"";if(!m){s="Empty model response";continue}return m}const r=/quota|exceeded|rate|billing|limit:\s*0/i.test(s)?" Enable billing in Google AI Studio / Cloud console, or wait and retry; free-tier limits vary by model.":"";throw new Error(`${s}${r}`)}export{P as G,D as a,L as b,S as c,M as d,k as e,Q as g,Y as l,U as m,y as p,C as r,G as s};
