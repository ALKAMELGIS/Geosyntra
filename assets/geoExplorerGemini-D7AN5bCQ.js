...
**Map-first (layer “show / find / on map” questions):** When the user asks to see, locate, or highlight a feature on the map, the app may move the map and open the feature card **before** your text appears. Lead with **what they are looking at** (layer + code/name) in **one or two short sentences** — do **not** open with row counts, “N record(s)”, or table dumps. If several attribute rows describe one geometry (1:N), mention that briefly at the end instead of listing duplicates.
...
`,a);return(r>=0?e.slice(0,r):e.slice(0,a)).trimEnd()}function G(n){return R(O(S(n))).replace(/\*\*([^*]+)\*\*/g,"$1").replace(/\*/g,"").trimEnd()}function I(n){const e=[];for(const t of n)if(t.type==="text")e.push({text:t.text});else if(t.type==="image")e.push({inline_data:{mime_type:t.mime,data:t.base64}});else{const a=t.table,r=a.columns.map(i=>i.label).join(" | "),o=a.rows.slice(0,12).map(i=>a.columns.map(u=>String(i.values[u.key]??"")).join(" | ")).join(`
`),s=`[Geo AI structured table omitted from vision — ${a.kind}: ${a.rows.length} rows. Columns: ${r}${o?`
Sample:
${o}`:""}]`;e.push({text:s})}return e}function U(n){return n.map(e=>({role:e.role,parts:I(e.parts)}))}function Y(n){for(let e=n.length-1;e>=0;e--){const t=n[e];if(t.role!=="model")continue;const a=y(A(t));if(a)return a}return null}function D(n){for(let e=n.length-1;e>=0;e--){const t=n[e];if(t.role!=="assistant"&&t.role!=="model")continue;const a=y(t.text);if(a)return a}return null}const v=["gemini-2.5-flash","gemini-2.5-flash-lite","gemini-flash-latest","gemini-2.5-pro","gemini-1.5-flash","gemini-1.5-flash-8b","gemini-1.5-pro"],N=["v1beta","v1"];function T(n,e){const t=`System (follow strictly):
${n}

---

`,a=e.map(s=>({role:s.role,parts:s.parts.map(i=>({...i}))})),r=a.findIndex(s=>s.role==="user");if(r<0)return[{role:"user",parts:[{text:t.trimEnd()}]},...a];const o=[...a[r].parts];if(o.length===0)o.push({text:t.trimEnd()});else{const s=o[0];typeof(s==null?void 0:s.text)=="string"?o[0]={text:t+s.text}:o.unshift({text:t.trimEnd()})}return a[r]={role:"user",parts:o},a}function _(n){const e=n.toLowerCase();return e.includes("api key not valid")||e.includes("invalid api key")||e.includes("invalid argument")&&e.includes("key")}function x(n,e){const t=e.toLowerCase();return n===404||n===400||n===403||n===429||n===503||t.includes("quota")||t.includes("exceeded")||t.includes("billing")||t.includes("limit: 0")||t.includes("resource_exhausted")||t.includes("resource exhausted")||t.includes("rate limit")||t.includes("rate_limit")||t.includes("overloaded")||t.includes("not found")||t.includes("is not found")||t.includes("not supported")||t.includes("permission_denied")||t.includes("permission denied")}async function Q(n){var s,i,u,d,h;const{apiKey:e,systemInstruction:t,contents:a}=n;let r="Unknown error";for(const E of v)for(const p of N){const g=`https://generativelanguage.googleapis.com/${p}/models/${E}:generateContent?key=${encodeURIComponent(e)}`,b=p==="v1beta"?{systemInstruction:{parts:[{text:t}]},contents:a}:{contents:T(t,a)},c=await fetch(g,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(b)}),l=await c.json().catch(()=>({}));if(!c.ok){if(r=((s=l==null?void 0:l.error)==null?void 0:s.message)||c.statusText||`HTTP ${c.status}`,_(String(r)))throw new Error(r);if(x(c.status,String(r)))continue;throw new Error(r)}const m=((h=(d=(u=(i=l==null?void 0:l.candidates)==null?void 0:i[0])==null?void 0:u.content)==null?void 0:d.parts)==null?void 0:h.map(w=>w.text).filter(Boolean).join(""))??"";if(!m){r="Empty model response";continue}return m}const o=/quota|exceeded|rate|billing|limit:\s*0/i.test(r)?" Enable billing in Google AI Studio / Cloud console, or wait and retry; free-tier limits vary by model.":"";throw new Error(`${r}${o}`)}export{P as G,D as a,L as b,S as c,M as d,k as e,Q as g,Y as l,U as m,y as p,C as r,G as s};
