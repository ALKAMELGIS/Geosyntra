...
/**
 * First sign-in for a configured system-owner email in development — creates the SQLite row
 * with the password the user typed (min 8 chars). Production never auto-creates accounts here.
 */
export function tryDevSystemOwnerFirstLogin(store, email, password) {
  if (isProductionDeployment()) return null
  const em = String(email || '').trim().toLowerCase()
  const pwd = String(password || '')
  if (!isSystemOwnerEmail(em) || pwd.length < 8) return null
  if (store.getUserByEmail?.(em)) return null

  const provisioned = store.provisionUserByOwner({
