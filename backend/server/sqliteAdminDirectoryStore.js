...
export function createSqliteAdminDirectoryStore(dbPath) {
  ...
  const getMeta = db.prepare(`SELECT value FROM admin_directory_meta WHERE key = ?`)
  const setMeta = db.prepare(
    `INSERT INTO admin_directory_meta (key, value) VALUES (?, ?)
     ON CONFLICT(key) DO UPDATE SET value = excluded.value`,
  )
  const delUserByEmail = db.prepare(`DELETE FROM admin_users WHERE email = ? COLLATE NOCASE`)

  function readDeletedEmails() {
    try {
      const row = getMeta.get('deletedEmails')
      if (!row?.value) return []
  ...
  function writeDeletedEmails(list) {
    const normalized = [...new Set((Array.isArray(list) ? list : []).map(e => normalizeEmail(e)).filter(Boolean))]
    setMeta.run('deletedEmails', JSON.stringify(normalized))
    return normalized
  }

  function mergeDeletedEmails(existing, incoming) {
    return writeDeletedEmails([...readDeletedEmails(), ...(Array.isArray(existing) ? existing : []), ...(Array.isArray(incoming) ? incoming : [])])
  }

  function applyDeletedEmails(deletedEmails) {
    const list = Array.isArray(deletedEmails) ? deletedEmails : readDeletedEmails()
    for (const em of list) {
      if (em) delUserByEmail.run(em)
    }
  }
  ...
  function readDirectory() {
    ...
    return {
      users,
      auditLog,
      deletedEmails: readDeletedEmails(),
  ...
  function writeDirectory({ users, auditLog, deletedEmails }) {
    const t = db.transaction(() => {
      db.exec(`DELETE FROM admin_audit`)
      if (deletedEmails !== undefined) {
        mergeDeletedEmails([], deletedEmails)
      }
      ...
      applyDeletedEmails(readDeletedEmails())
    })
    t()
  ...
  function readPublicDirectory() {
    ...
    return {
      ...
      deletedEmails: internal.deletedEmails || readDeletedEmails(),
