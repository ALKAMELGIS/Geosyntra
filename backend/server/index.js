import { bootstrapServerEnvironment } from './bootstrapEnv.js'
...
import { registerPassportOAuthRoutes } from './registerPassportOAuthRoutes.js'
import { registerOAuthPublicRoutes } from './registerOAuthPublicRoutes.js'
import { isProviderConfigured } from './oauth/oauthConfig.js'
...
import { bootstrapSystemOwners } from './rbac/systemOwnerUser.js'
import { ensureSystemOwnerAccounts } from './rbac/ensureSystemOwnerAccounts.js'
...
import { registerApiGatewayRoutes } from './gateway/registerApiGatewayRoutes.js'
import { registerApiConfigRoutes } from './config/registerApiConfigRoutes.js'
import { registerPlatformEnvRoutes } from './config/registerPlatformEnvRoutes.js'
import { isProductionDeployment } from './bootstrapEnv.js'
import { registerTokenRevisionBroadcast, bumpTokenRevision } from './tokenManager/tokenRevision.js'
import { startEnvConfigReload } from './envConfigReload.js'
...
bootstrapServerEnvironment({ repoRoot: REPO_ROOT })

const PLATFORM_PATHS = resolvePlatformPaths({ serverDir: SERVER_DIR, repoRoot: REPO_ROOT })
const PLATFORM_LAYOUT = ensurePlatformDataLayout(PLATFORM_PATHS, REPO_ROOT)

const API_SECRETS_FILE = PLATFORM_PATHS.apiSecretsFile
const USER_PROFILES_FILE = PLATFORM_PATHS.userProfilesFile
...
registerPlatformRoutes(app, PLATFORM_PATHS, PLATFORM_LAYOUT)
...
app.post('/api/auth/google/exchange', async (req, res) => {
  try {
    ...
    if (!result.ok) {
      const status = result.error === 'oauth_google_missing_config_or_code' ? 400 : 401
...
/** Apple Sign In — authorization `code` or `identity_token`. */
app.post('/api/auth/apple/exchange', async (req, res) => {
  try {
    const result = await exchangeAppleAuthPayload(req.body)
    if (!result.ok) {
      const status =
        result.error === 'apple_oauth_missing_config_or_token' ||
        result.error === 'apple_oauth_missing_server_keys' ||
        result.error === 'apple_oauth_missing_redirect'
          ? 400
          : 401
      return res.status(status).json(result)
    }
...
/** GitHub OAuth — login (scope user:email). */
app.post('/api/auth/github/exchange', async (req, res) => {
  try {
    const code = String(req.body?.code || '').trim()
    const redirectUri = String(
      req.body?.redirect_uri || process.env.AUTH_GITHUB_REDIRECT_URI || '',
    ).trim()
    const result = await exchangeGitHubAuthCode(code, redirectUri)
    if (!result.ok) {
      const status = result.error === 'oauth_github_missing_config_or_code' ? 400 : 401
      return res.status(status).json(result)
    }
    return res.json(result)
  } catch (e) {
    return res.status(500).json({ ok: false, error: 'oauth_github_server_error' })
  }
})
...
app.post('/api/auth/linkedin/exchange', async (req, res) => {
  try {
    const code = String(req.body?.code || '').trim()
    const redirectUri = String(
      req.body?.redirect_uri || process.env.LINKEDIN_OAUTH_REDIRECT_URI || process.env.GOOGLE_OAUTH_REDIRECT_URI || '',
    ).trim()
    const result = await exchangeLinkedInAuthCode(code, redirectUri)
    if (!result.ok) {
      const status = result.error === 'oauth_linkedin_missing_config_or_code' ? 400 : 401
      return res.status(status).json(result)
    }
    return res.json(result)
  } catch (e) {
    return res.status(500).json({ ok: false, error: 'oauth_linkedin_server_error' })
  }
})
...
void platformTokenApi.tokenManager
  .migrateFromLegacyVault('system@boot')
  .then(vaultMigrate => {
...
registerGeoGroundingRoutes(app, {
  secretsFilePath: API_SECRETS_FILE,
  getSystemToken: name => platformTokenApi.tokenManager.getSystemToken(name),
  invokeMiddleware: [
    optionalAuthOrFree(() => authDirectoryStore, () => billingApi.subscriptionStore),
    (req, _res, next) => {
      req.subscriptionStore = billingApi.subscriptionStore
      next()
    },
...
if (promoteOwnerEmail) {
  try {
    ...
    if (promoted.ok && !promoted.unchanged) {
      ...
      console.warn('[rbac] RBAC_PROMOTE_OWNER_EMAIL user not found:', promoteOwnerEmail)
    }
  } catch (e) {
    console.error('[rbac] promote owner error', e)
  }
}
...
app.post('/api/ai/chat', async (req, res) => {
  ...
  const resolvedOpenAiKey =
    (await platformTokenApi.tokenManager.getSystemToken('openai')) || OPENAI_API_KEY || ''
  const openaiClient = resolvedOpenAiKey ? new OpenAI({ apiKey: resolvedOpenAiKey }) : null
...
const server = app.listen(3001, () => {
  ...
  console.log(
    `[auth] email: ${hasEmailConfig() ? emailProviderLabel() : 'not configured (set SMTP_* or RESEND_*)'}`,
  )
  console.log(
    `[auth] oauth: google=${isProviderConfigured('google')} linkedin=${isProviderConfigured('linkedin')} github=${isProviderConfigured('github')} apple=${isProviderConfigured('apple')}`,
  )
})
...
registerTokenRevisionBroadcast(({ revision, reason }) => {
  broadcast({
    topic: 'platform/tokens',
    revision,
    reason,
    at: new Date().toISOString(),
  })
