...
export async function proxyMapboxRequest(req, res, accessToken) {
  ...
  try {
    ...
    // Mapbox tiles/glyphs/sprites/fonts are versioned and effectively immutable. The old
    // `private, max-age=60` forced the browser to refetch every tile through this proxy each
    // minute, which made panning/zooming feel slow. Cache binary assets aggressively (publicly,
    // since the imagery is not user-specific) and keep style/TileJSON responses on a short TTL.
    ...
    res.setHeader(
      'Cache-Control',
      isImmutableAsset ? 'public, max-age=604800, immutable' : 'public, max-age=3600',
    )
    if (isImmutableAsset) {
      res.setHeader('CDN-Cache-Control', 'max-age=604800')
      res.setHeader('Cloudflare-CDN-Cache-Control', 'max-age=604800')
    }
