...
export function passwordResetExpiresAt(fromMs = Date.now()) {
  return new Date(fromMs + PASSWORD_RESET_TTL_MS).toISOString()
}

export function isPasswordResetExpired(expiresAt) {
  return isVerificationExpired(expiresAt)
}

/**
 * @returns {{ ok: true } | { ok: false; retryAfterSec: number }}
 */
export function checkPasswordResetCooldown(email) {
...
export function markPasswordResetSent(email) {
  resendLastSent.set(`reset:${normalizeEmail(email)}`, Date.now())
}
