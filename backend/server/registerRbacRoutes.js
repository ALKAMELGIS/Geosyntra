...
export function registerRbacRoutes(app, deps) {
  ...
  const invites = createInviteStore(deps.platformDb ?? deps.sqlitePath)
  ...
  app.get('/api/rbac/users', requireAuth, requirePermission(PERMISSIONS.USERS_READ), async (_req, res) => {
    const users = (await storeAwait(store.listUsers())).map(u => toPublicAuthUser(u))
    return res.json({ ok: true, users })
  })

  app.get('/api/rbac/audit', requireAuth, requirePermission(PERMISSIONS.AUDIT_READ), async (req, res) => {
    const limit = Math.min(500, Math.max(1, Number(req.query?.limit) || 100))
    return res.json({ ok: true, audit: await storeAwait(store.getAuditLog(limit)) })
  })

  app.post('/api/rbac/users/:id/approve', requireAuth, requirePermission(PERMISSIONS.USERS_APPROVE), async (req, res) => {
    const id = Number(req.params.id)
    const result = await storeAwait(store.approveUser(id, req.authUser))
  ...
  app.post('/api/rbac/users/:id/suspend', requireAuth, requirePermission(PERMISSIONS.USERS_SUSPEND), async (req, res) => {
    const id = Number(req.params.id)
    const result = await storeAwait(store.suspendUser(id, req.authUser))
    if (!result?.ok) return res.status(result?.error === 'not_found' ? 404 : 400).json(result)
    deps.addAuthEvent('rbac_user_suspended', { targetId: id, by: req.authUser.email })
    return res.json({ ok: true })
  })

  app.post('/api/rbac/users/:id/reactivate', requireAuth, requirePermission(PERMISSIONS.USERS_SUSPEND), async (req, res) => {
    const id = Number(req.params.id)
    const result = await storeAwait(store.reactivateUser(id, req.authUser))
    if (!result?.ok) return res.status(result?.error === 'not_found' ? 404 : 400).json(result)
    return res.json({ ok: true })
  })

  app.delete('/api/rbac/users/:id', requireAuth, requirePermission(PERMISSIONS.USERS_MANAGE), async (req, res) => {
    const id = Number(req.params.id)
    if (!Number.isFinite(id) || id <= 0) {
      return res.status(400).json({ ok: false, error: 'invalid_id' })
    }
    const result = await storeAwait(store.deleteUser(id, req.authUser))
    if (!result?.ok) {
      const status =
        result?.error === 'not_found'
          ? 404
          : result?.error === 'last_owner' || result?.error === 'cannot_delete_self' || result?.error === 'protected_account'
            ? 400
            : 400
      return res.status(status).json(result)
    }
    deps.addAuthEvent('rbac_user_deleted', { targetId: id, email: result.email, by: req.authUser.email })
    return res.json({ ok: true, email: result.email })
  })

  app.patch('/api/rbac/users/:id', requireAuth, requirePermission(PERMISSIONS.ROLES_ASSIGN), async (req, res) => {
    const id = Number(req.params.id)
    ...
    const result = await storeAwait(store.setUserRole(id, roleSlug, actor))
    if (!result?.ok) {
  ...
  app.get('/api/rbac/invites', requireAuth, requirePermission(PERMISSIONS.INVITES_CREATE), async (_req, res) => {
    return res.json({ ok: true, invites: await storeAwait(invites.list(100)) })
  })
  ...
  app.post('/api/rbac/invites', requireAuth, requirePermission(PERMISSIONS.INVITES_CREATE), async (req, res) => {
    try {
      ...
      const created = await storeAwait(
  ...
  app.get('/api/rbac/invites/preview', async (req, res) => {
    const token = String(req.query?.token || '').trim()
    if (!token) return res.status(400).json({ ok: false, error: 'token_required' })
    const invite = await storeAwait(invites.getByToken(token))
  ...
  app.post('/api/rbac/invites/accept', async (req, res) => {
    try {
      ...
      const accepted = await storeAwait(invites.acceptInvite(token))
      if (!accepted.ok) {
      ...
      if (await storeAwait(store.getUserByEmail(invite.email))) {
        return res.status(409).json({ ok: false, error: 'email_exists' })
      }
      const created = await storeAwait(
