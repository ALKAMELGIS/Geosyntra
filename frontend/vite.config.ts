...
function ghPagesHashAndSlashRedirect(): Plugin {
  ...
  const snippet = `<script ${marker}="1">;(function(){try{var h=String(location.hostname||"");if(h.indexOf("github.io")===-1)return;var p=location.pathname||"";var ws=${JSON.stringify(withSlash)};var ns=${JSON.stringify(noSlash)};var g=location.hash||"";if(g.startsWith("#/")){var route=g.slice(2).split("?")[0].replace(/\\/$/,"");if(route==="app/onboarding/trial-start"){location.replace(location.origin+ws+location.search+"/#/?start=1&wizard=pricing");return;}if(route==="app/auth/login"){location.replace(location.origin+ws+location.search+"/#/?start=1&wizard=auth&mode=signin");return;}if(route==="app/auth/register"){location.replace(location.origin+ws+location.search+"/#/?start=1&wizard=auth&mode=signup");return;}}if(g&&g!=="#/"&&!g.startsWith("#/")){try{sessionStorage.setItem("geosyntra-scroll-to",g.slice(1));}catch(_){}location.replace(location.origin+ws+location.search+"#/");return;}if(p===ns){location.replace(location.origin+ws+location.search+(g&&g.length>1?g:"#/"));return;}if((p===ws||p===ns+"/")&&(!g||g==="#")){location.replace(location.origin+ws+location.search+"#/");}}catch(_){}})();</script>`
  return {
