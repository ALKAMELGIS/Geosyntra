...
/** Axum dev API (default :3003). Override: VITE_DEV_API_PROXY=http://127.0.0.1:3003 */
const devApiProxyTarget =
  process.env.VITE_DEV_API_PROXY?.trim() ||
  process.env.GEOSYNTRA_API_PROXY?.trim() ||
  'http://127.0.0.1:3003'
...
export default defineConfig({
  ...
  preview: {
    ...
    // Match dev server so `npm run preview` + Node API still persist API vault to geosyntra_api_secrets.json.
    proxy: {
      '/__mapbox': mapboxDevProxyOptions(),
      '/api/analysis-engine': {
        target: 'http://127.0.0.1:8000',
