...
export async function orsMatrixMetrics(args: {
  ...
  const norm = (m: (number | null)[][]) =>
    m.map(row =>
      row.map(v => (v != null && Number.isFinite(v) ? v : Number.POSITIVE_INFINITY)),
    )

  return { durations: norm(durations), distances: norm(distances) }
}
