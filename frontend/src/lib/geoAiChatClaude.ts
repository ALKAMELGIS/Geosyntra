...
export async function buildGeoAiDataContext(
  maxChars = 48000,
  opts?: {
    satelliteLayers?: GeoAiMapLayer[]
    includeGisSavedLayers?: boolean
  ...
  if (opts?.layerRegistryBlock?.trim()) {
    chunks.push(
      '### MAP LAYER REGISTRY (all loaded layers — vector, raster, WMS, imagery)\n' +
        opts.layerRegistryBlock.trim().slice(0, Math.min(32000, maxChars - 8000)),
    )
  }
