...
export function resolveGeographicPlaceFromQuery(query: string): string | null {
  ...
  if (mOnMap?.[1]) {
    ...
    if (place.length >= 2 && !isLikelyLayerOrRsPhrase(place)) return place
  }
  ...
  for (const re of patterns) {
    ...
    if (place.length >= 2 && place.length <= 120 && !isLikelyLayerOrRsPhrase(place)) return place
  }
  return null
}
    ...
    case 'zonal_stats':
      if (!context.aoi) {
        return {
          ok: false,
          reason:
            'Draw or select an **AOI** on the map first (polygon tool in AI Detection or Fields), then ask for analysis again.',
