...
describe('geoAiAgentIntent', () => {
  ...
  it('does not treat layer asset ids as map places when layer is mentioned', () => {
    const q = 'From layer Agro_Structures Show me MH105 ON MAP'
    expect(extractMapPlaceText(q)).toBeNull()
    expect(resolveGeographicPlaceFromQuery(q)).toBeNull()
  })
