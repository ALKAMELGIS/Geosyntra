...
function publicSiteHostname(): string {
  if (typeof window === 'undefined') return ''
  return window.location.hostname.toLowerCase()
}

/** GeoSyntra production host (GitHub Pages custom domain or Hostinger). */
export function isGeosyntraPublicSite(): boolean {
  const host = publicSiteHostname()
  return (
    host === 'geosyntra.org' ||
...
/** True when the SPA is served without a workspace auth API (e.g. GitHub Pages). */
export function isStaticLocalAuthMode(): boolean {
  if (isAuthApiConfigured()) return false
  if (typeof window === 'undefined') return true
  const host = publicSiteHostname()
  if (host === 'localhost' || host === '127.0.0.1') return false
  if (isGeosyntraPublicSite()) return true
