...
> {
  ...
  if (status === 0 || data.error === 'network_error') {
    return {
      ok: false,
      error:
        'Cannot reach the auth server. Start Axum on port 3003 (cargo run -p geosyntra-api) and refresh.',
    }
  }
...
function normalizeLoginUser(
  ...
  if (!raw || typeof raw.email !== 'string' || !raw.email.trim()) return null
  const idRaw = raw.id
  const id =
    typeof idRaw === 'number'
      ? idRaw
      : typeof idRaw === 'string'
        ? Number.parseInt(idRaw, 10) || Date.now()
        : Date.now()
  ...
  return {
    ...
    role: String(raw.role ?? 'Trial User'),
    roleSlug:
      typeof raw.roleSlug === 'string'
        ? raw.roleSlug
        : typeof raw.role_slug === 'string'
          ? raw.role_slug
          : undefined,
    status,
    emailVerified,
...
> {
  ...
  const errCode = String(data.error ?? data.code ?? '')
  ...
  if (status === 403 && errCode === 'pending_approval') {
    return {
      ok: false,
  ...
  if ((status === 401 || status === 400) && errCode === 'user_not_found') {
    return {
      ok: false,
      error:
        data.message ||
  ...
  if ((status === 401 || status === 400) && errCode === 'invalid_password') {
    return { ok: false, error: data.message || 'Incorrect password.' }
  }
  if (status === 403 && errCode === 'auth_incomplete') {
    return {
      ok: false,
      error:
        data.message ||
  ...
  if (status === 0 || errCode === 'network_error') {
    return {
      ok: false,
      error:
        'Cannot reach the auth server. Start Axum on port 3003 (cargo run -p geosyntra-api) and refresh.',
