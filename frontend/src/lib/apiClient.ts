...
/** In local dev, route /api through the Vite proxy when the configured API host is localhost. */
function devProxyApiBase(configured: string): string {
  ...
  try {
    ...
    if (isLocalHostname(api.hostname)) return ''
    return configured
  } catch {
    return configured
  }
...
export function describeWorkspaceFetchError(error: unknown): string {
  ...
  if (lower.includes('failed to fetch') || lower.includes('networkerror') || lower.includes('network request failed')) {
    if (import.meta.env.DEV) {
      return 'Cannot reach the GeoSyntra API. Start Axum (port 3003): cargo run -p geosyntra-api — or bash scripts/dev-axum-with-frontend.sh'
    }
