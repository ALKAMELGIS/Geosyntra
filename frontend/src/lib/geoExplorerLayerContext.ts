...
export function buildGeoAiLayerPopupAttributeRows(
  ...
  const displayed: Record<string, string> =
    arc && Object.keys(p).length
      ? formatFeaturePropertiesForPopup(p as Record<string, unknown>, ft, arc)
      : Object.fromEntries(
          Object.entries(p).map(([k, v]) => [
            k,
            sanitizePopupDisplayValue(v === null || v === undefined ? '—' : String(v)),
          ]),
        )
...
export type LayerAttributeSearchMatch = LayerQueryMatch & {
  layerId: string
  featureKey: string
  featureIndex: number
  label: string
...
function layerAttributeSearchLabel(hit: LayerQueryMatch): string {
  const place = pickGeoAiHumanPlaceFields(hit.properties)
  const fromProps = String(
    place.areaName ??
      hit.properties?.Farm_Code ??
      hit.properties?.Name ??
      hit.properties?.name ??
      hit.properties?.OBJECTID ??
      '',
  ).trim()
  return fromProps || hit.layerName
}

function collectLayerAttributeSearchMatches(
  userText: string,
  layers: GeoAiMapLayer[],
  opts?: { layerNameHint?: string | null; minScore?: number; limit?: number },
): LayerAttributeSearchMatch[] {
  const minScore = opts?.minScore ?? 32
  const limit = opts?.limit ?? 8
  const hint = opts?.layerNameHint ?? extractLayerHint(userText, layers)
  const hintNorm = hint ? normalizeLayerName(hint) : ''
  const scopedLayers = hintNorm
    ? layers.filter(l => {
        const ln = normalizeLayerName(l.name)
        const lnSp = normalizeLayerName(l.name.replace(/_/g, ' '))
        return (
          ln === hintNorm ||
          ln.includes(hintNorm) ||
          hintNorm.includes(ln) ||
          lnSp.includes(hintNorm) ||
          hintNorm.includes(lnSp)
        )
      })
    : layers
  ...
  const seen = new Set<string>()
  const out: LayerAttributeSearchMatch[] = []

  const pushMatch = (layer: GeoAiMapLayer, f: GeoAiFeature, featureIndex: number, hit: LayerQueryMatch) => {
    if (hit.score < minScore) return
    const layerId = String(layer.clientLayerId ?? layer.name)
    const featureKey = computeStableGisFeatureKey(f, featureIndex)
    const dedupeKey = `${layerId}::${featureKey}`
    ...
    out.push({
      ...
      featureKey,
      featureIndex,
      label: layerAttributeSearchLabel(hit),
    })
  }

  const exact = findExactLayerAttributeMatchInLayers(userText, scopedLayers)
  if (exact) {
    for (const layer of scopedLayers) {
      ...
      for (let i = 0; i < fc.features.length; i++) {
        ...
        const c = featureCentroid(f)
        if (!c || Math.abs(c[0] - exact.lng) > 1e-5 || Math.abs(c[1] - exact.lat) > 1e-5) continue
        pushMatch(layer, f, i, exact)
        break
      }
    }
  }

  const nameScoped = !hintNorm ? layersWhoseNameAppearsInQuery(userText, scopedLayers) : null
  const tokens = tokenizeForSearch(userText, scopedLayers)
  const noise = /^(the|map|maps|layer|layers|show|find|where|gps|geo|json|data|row|rows|please)$/i
  const substringCandidates = [...new Set(tokens)]
    .filter(t => t.length >= 3 && t.length <= 64 && !noise.test(t))
    .sort((a, b) => b.length - a.length)
  ...
  for (const layer of scopedLayers) {
    ...
    for (let i = 0; i < fc.features.length; i++) {
      ...
      if (substringCandidates.length) {
        const p = f.properties
        if (p && typeof p === 'object') {
          let localBest = 0
          for (const tok of substringCandidates) {
            for (const [key, val] of Object.entries(p)) {
              if (val == null) continue
              const vs = String(val).trim()
              if (vs.length < 2) continue
              localBest = Math.max(localBest, scoreSubstringTokenInValue(tok, key, vs))
            }
          }
          score = Math.max(score, localBest)
        }
      }
      if (hintNorm) score += 12
      ...
      const fullprops =
        f.properties && typeof f.properties === 'object'
          ...
          : null
      const summary =
        fullprops && Object.keys(fullprops).length
          ? JSON.stringify(
              arcDef ? formatFeaturePropertiesForGeoAi(fullprops, f, arcDef) : fullprops,
            ).slice(0, 220)
          : 'geometry match'
...
/** Multi-hit attribute search across loaded vector layers (map search bar, autocomplete). */
export function findAllLayerAttributeSearchMatches(
  userText: string,
  layers: GeoAiMapLayer[],
  opts?: { limit?: number; minScore?: number; layerNameHint?: string | null },
): LayerAttributeSearchMatch[] {
  const q = userText.trim()
  if (!q || !layers.length) return []
  const layerHint = opts?.layerNameHint ?? extractGeoExplorerLayerHint(q, layers)
  return collectLayerAttributeSearchMatches(q, layers, {
    layerNameHint: layerHint,
