...
export function formatApiTokenSaveError(raw: string | undefined): string {
  ...
  if (lower.includes('token_store_unavailable') || lower.includes('no_db')) {
    return 'Token database is unavailable on the API server. On Hostinger, set AGRI_DATA_DIR to a writable folder, restart the Node app, and ensure SQLite migrations ran.'
  }
  ...
  if (lower.includes('invalid or missing x-agri-api-secrets-token')) {
    return 'API vault rejected the request. Remove AGRI_API_SECRETS_TOKEN on the server or sign in as Owner so JWT auth is accepted.'
  }
  ...
  if (lower.includes('token_persist_failed')) {
    return 'Server could not write the token to SQLite. Check AGRI_DATA_DIR permissions and restart the Node app on Hostinger.'
  }
