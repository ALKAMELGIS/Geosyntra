...
export async function computeSiRouteSession(args: {
  ...
  // When a routing provider IS configured but the request fails (e.g. ORS "Could not find a
  // routable point" for endpoints far from any road), keep the real error so the UI can show it
  // instead of the misleading "add/update your API key" hint that only applies when nothing is
  // configured. Only a fully unconfigured stack should fall through to that hint (null return).
  ...
  if (orsKey) {
    try {
      ...
      providerError = e instanceof Error ? e : new Error(String(e))
  ...
  if (ghKey) {
    try {
      ...
      providerError = e instanceof Error ? e : providerError
      console.warn('[graphhopper] client route failed, trying server:', e)
    }
  }

  let resp: Awaited<ReturnType<typeof groundingComputeRoute>> = null
  try {
    resp = await groundingComputeRoute({
