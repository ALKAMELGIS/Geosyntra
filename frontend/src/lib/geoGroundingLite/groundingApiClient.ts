...
export async function fetchGeoGroundingStatus(): Promise<{ configured: boolean; tools: string[] }> {
  const base = apiBase()
  ...
  try {
    ...
    return { configured: Boolean(data.configured), tools: Array.isArray(data.tools) ? data.tools : [] }
  } catch {
