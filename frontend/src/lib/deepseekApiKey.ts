/**
 * DeepSeek — production uses `/api/gateway/deepseek/*`.
 */
...
function readLsDeepseekKey(): string {
...
export function getDeepseekApiKeyBrowserOverride(): string {
  return readBuiltinBrowserOverride(readLsDeepseekKey)
}

export function getDeepseekApiKey(): string {
  const fromLs = getDeepseekApiKeyBrowserOverride()
  if (fromLs) return fromLs
  return readBuiltinEnvFallback(envDeepseekKey())
}
