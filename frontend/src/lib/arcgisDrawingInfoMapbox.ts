...
function uniqueValueInfoKeys(uvi: any): string[] {
...
export function arcgisLayerUsesServiceSymbology(layer: {
  ...
  if (!layer.arcgisDrawingInfo) return false;
  if (layer.symbology?.useArcGisOnline !== true && layer.useArcGisSymbology !== true) return false;
  return arcgisDrawingInfoSupportsMapboxRender(layer.arcgisDrawingInfo);
}
