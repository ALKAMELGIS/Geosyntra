...
/** Strip internal code suffixes from legacy / LLM-formatted attribute strings. */
export function sanitizePopupDisplayValue(value: string): string {
  return String(value ?? '')
    .replace(/\s*\(stored code:\s*[^)]+\)\s*$/i, '')
    .replace(/\s*\(code:\s*[^)]+\)\s*$/i, '')
    .trim()
}

/** Human-readable attribute values for map identify popups — labels only, no stored codes. */
export function formatFeaturePropertiesForPopup(
  ...
  for (const key of Object.keys(props)) {
    ...
    let text = parts.description || parts.display || parts.code || ''
    if (!text && raw !== null && raw !== undefined) text = String(raw)
    out[key] = sanitizePopupDisplayValue(text)
  }
  return out
}

/** ArcGIS Online–style popup title: "Layer name: Feature name". */
export function resolveIdentifyPopupTitle(
  properties: Record<string, unknown> | null | undefined,
  layerTitle: string,
): string {
  const layer = String(layerTitle ?? '').trim() || 'Feature'
  if (!properties || typeof properties !== 'object') return layer
  const candidates = [
    'Farm_Name',
    'FARM_NAME',
    'farm_name',
  ...
  for (const k of candidates) {
    ...
    if (layer.toLowerCase() === s.toLowerCase()) return layer
    return `${layer}: ${s}`
  }
  return layer
}
