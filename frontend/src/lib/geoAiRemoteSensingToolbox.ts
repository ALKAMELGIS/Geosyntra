...
export function tryGeoAiRemoteSensingToolboxAction(ctx: {
  ...
  /* WMS visibility */
  if (/\b(hide|disable|turn off)\b/i.test(query) && /\b(layer|overlay|wms|imagery|raster|satellite|on\s+map)\b/i.test(query)) {
    pushUnique({ kind: 'setWmsOverlayVisible', visible: false })
    notes.push('Turned off the RS imagery overlay on the map.')
  } else if (
    /\b(show|enable|turn on)\b/i.test(query) &&
    /\b(layer|overlay|wms|ndvi|imagery|raster|satellite|on\s+map)\b/i.test(query)
  ) {
    pushUnique({ kind: 'setWmsOverlayVisible', visible: true })
    notes.push('Enabled **Show … on map** for the active RS layer.')
  }
