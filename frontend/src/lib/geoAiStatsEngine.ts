...
import {
  isGeoExplorerPlaceDiscoveryQuestion,
  spatialWorkflowOverridesTabularStats,
} from './geoAiSatelliteSpatialWorkflow'
...
export function runGeoAiStatsCommand(query: string, layers: GeoAiMapLayer[]): GeoAiStatsResult | null {
  const q = query.trim()
  if (!q) return null
  if (isGeoExplorerPlaceDiscoveryQuestion(q)) return null
  if (isMapPlaceShowOrGeocodeQuery(q) || resolveGeographicPlaceFromQuery(q)) return null
  const agentIntent = detectGeoAiAgentIntent(q)
  if (agentIntent.type === 'map_place' || agentIntent.type === 'route' || agentIntent.type === 'places_poi') {
    return null
  }
  ...
  const hasStatIntent =
    ...
    (/\b(show\s+me|find|display|list|locate|highlight)\b/i.test(q) &&
      !resolveGeographicPlaceFromQuery(q) &&
      !isMapPlaceShowOrGeocodeQuery(q)) ||
    /\b(records|features)\b/i.test(q) ||
  ...
  if (groupByField) {
    ...
    return {
      handled: true,
      reply: `Group By **${gb}** over **${selectedCount}** rows (${buckets.size} groups). Interactive table and chart below.`,
      table,
    }
  }
