 ...
 */
type HeroThemeToggleProps = {
  /** `nav` — home header. `wizard` — sign-in row before “Keep me signed in”. `fixed` — legacy floating pill. */
  placement?: 'fixed' | 'nav' | 'wizard'
}

export default function HeroThemeToggle({ placement = 'fixed' }: HeroThemeToggleProps) {
  const inNav = placement === 'nav'
  const inWizard = placement === 'wizard'
  const inline = inNav || inWizard
  ...
  }, [inline, mouseOffset.x, mouseOffset.y, scrolled])
  ...
  return (
    <button
      ...
      className={
        inWizard
          ? 'hero-theme-toggle hero-theme-toggle--wizard'
          : inNav
            ? 'hero-theme-toggle hero-theme-toggle--nav'
            : 'hero-theme-toggle'
      }
      style={{
        transform: transformStyle,
        opacity: inline ? 1 : scrolled ? 0.85 : 1,
      }}
    >
