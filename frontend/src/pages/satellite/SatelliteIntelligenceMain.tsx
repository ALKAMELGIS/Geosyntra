...
export default function SatelliteIntelligence() {
  ...
  const siStyleProjectionSyncBusyRef = useRef(false);
  ...
  const siSyncMapProjection = useCallback(() => {
    if (siProjectionSwitchingRef.current || siStyleProjectionSyncBusyRef.current) return;
    const mapInstance = mapRef.current?.getMap?.() ?? mapRef.current;
    if (!mapInstance) return;
    const mode = mapProjectionModeRef.current;
    const wantProjection = mode === 'globe' ? 'globe' : 'mercator';
    try {
      if (readSiMapboxProjectionName(mapInstance) !== wantProjection) {
        mapInstance.setProjection({ name: wantProjection });
      }
    } catch {
      /* ignore */
    }
  ...
  const handleMapStyleData = useCallback(() => {
    ...
    siStyleProjectionSyncBusyRef.current = true;
    try {
      siSyncMapProjection();
    } finally {
      siStyleProjectionSyncBusyRef.current = false;
    }
  }, [siSyncMapProjection]);
  ...
  const synthesizeWeeklyComposites = (itemCount: number) => {
    const range = selectedIndexConfig.range ?? ([-1, 1] as [number, number]);
    ...
    if (mapDrawTool === 'select') {
      try {
        ...
        if (map?.project && map.queryRenderedFeatures) {
          ...
          const isWorkspaceMultiHit = (h: (typeof hits)[number] | undefined) => {
            if (!h) return false;
            const lid = String(h.layer?.id ?? '');
            if (!lid.startsWith('si-multi-aoi-')) return false;
            if (lid === 'si-multi-aoi-cluster' || lid === 'si-multi-aoi-cluster-count') return false;
            const rp =
              h.properties && typeof h.properties === 'object' && !Array.isArray(h.properties)
                ? (h.properties as Record<string, unknown>)
                : {};
            return String(rp.aoiId ?? '').trim().length > 0;
          };
          const hit = hits.find(isWorkspaceMultiHit) ?? hits[0];
          ...
          if (hit && layerId && !siIdentifyLayerIsSkippable(layerId)) {
            ...
            if (layerId.startsWith('drawn-index-geometry')) {
              ...
              if (items.length > 0) {
                ...
                if (drawn) {
                  const matches = items.filter(r => featureGeometriesMatch(drawn, r.feature));
                  if (matches.length === 1) pickId = matches[0]!.id;
                }
                if (!pickId && items.length === 1) pickId = items[0]!.id;
                if (pickId) {
                  const row = items.find(x => x.id === pickId);
                  if (row) {
                    setActiveMultiAoiId(pickId);
                    updateDrawnStats(row.feature);
                    setFieldAnalysisStatus(`Selected ${row.name}. Tap Clear drawing to remove this AOI.`);
                  }
                  return;
                }
              }
  ...
  const mpcZonalApiLayerIds = useMemo(
    () => popupZonalLayerIds.filter(id => id !== 'LST'),
    [popupZonalLayerIds],
  );
  ...
  const satelliteHasClearableDrawing = useMemo(() => {
    const rsInFlightSketch =
      ...
      (rectCirclePreview != null ||
        ...
        circleRefineDraft != null ||
        polylineStart != null);
    const rsToolActive = !fieldsPanelDrawArmed && mapDrawTool !== 'select';
    return (
      drawnGeometry != null ||
      aoiFields.length > 0 ||
      ...
      <div className="si-main-content">
        ...
        >
          ...
          <MapGL
            ...
            {...mapViewState}
            onMove={evt => {
              if (isSiViewportChangeBlocked() || siProjectionSwitchingRef.current) return;
              setViewState(
                clampSiViewStateForProjection(evt.viewState, mapProjectionModeRef.current),
              );
            }}
                        ...
                        <div className="si-field-analysis-section">
                          ...
                          <div className="si-field-analysis-map-tools">
                            ...
                            >
                              <button
                                type="button"
                                className={`si-map-analysis-tool${!fieldsPanelDrawArmed && mapDrawTool === 'rectangle' ? ' si-map-analysis-tool--on' : ''}`}
                                title="Rectangle AOI"
                                aria-pressed={!fieldsPanelDrawArmed && mapDrawTool === 'rectangle'}
                              ...
                              <button
                                type="button"
                                className={`si-map-analysis-tool${!fieldsPanelDrawArmed && mapDrawTool === 'polygon' ? ' si-map-analysis-tool--on' : ''}`}
                                title="Polygon AOI"
                                aria-pressed={!fieldsPanelDrawArmed && mapDrawTool === 'polygon'}
                              ...
                              <button
                                type="button"
                                className={`si-map-analysis-tool${!fieldsPanelDrawArmed && mapDrawTool === 'circle' ? ' si-map-analysis-tool--on' : ''}`}
                                title="Circle AOI"
                                aria-pressed={!fieldsPanelDrawArmed && mapDrawTool === 'circle'}
                              ...
                              <button
                                type="button"
                                className={`si-map-analysis-tool${!fieldsPanelDrawArmed && mapDrawTool === 'select' ? ' si-map-analysis-tool--on' : ''}`}
                                title={drawnGeometry ? 'Select / edit AOI' : 'Select'}
                                aria-pressed={!fieldsPanelDrawArmed && mapDrawTool === 'select'}
                              ...
                              <>
                                ...
                                <ul className="si-rs-aoi-workspace-list" aria-label="Workspace AOIs">
                                  {multiAoiItems.map(row => {
                                    const layerIds = remoteSensingLayerOptions.map(o => o.id);
                                    const vis = siMergedAoiLayerVisibility(
                                      row.rasterStackVisible,
                                      layerIds,
                                      remoteSensingLayerOptions,
                                    );
                                      ...
                                      >
                                        ...
                                        <div className="si-rs-aoi-workspace-card__body">
                                          <SiAoiWorkspaceSettingsPanel
                                            ...
                                            layersOnCount={remoteSensingLayerOptions.reduce(
                                              (n, o) => n + (vis[o.id] ? 1 : 0),
                                              0,
                                            )}
                                            layersTotal={remoteSensingLayerOptions.length}
                                            aoiId={row.id}
                                            layersSlot={
                                              remoteSensingLayerOptions.length === 0 ? (
