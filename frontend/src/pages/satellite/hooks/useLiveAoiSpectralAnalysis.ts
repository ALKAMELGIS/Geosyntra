...
export function useLiveAoiSpectralAnalysis(opts: UseLiveAoiSpectralAnalysisOpts) {
  ...
  const mpcLayerIds = useMemo(() => {
    const fromPopup = mpcZonalApiLayerIdsFromPopup(opts.layerIds.length ? opts.layerIds : LIVE_AOI_MPC_LAYER_IDS);
    const merged = new Set<StaticAoiChartLayerId>([...fromPopup, activeLayerId].filter(id => id !== 'LST'));
    return [...merged];
