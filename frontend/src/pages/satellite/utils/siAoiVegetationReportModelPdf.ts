...
import type {
  ...
  SiAoiReportModel,
  SiAoiReportTimePoint,
} from './siAoiVegetationReportModel'
import type { SiAoiAgriculturalInterpretation } from './siAoiAgriculturalInterpretation'
import { formatLatestImageryDateLine } from './siAoiAgriculturalInterpretation'
import { buildWeeklyPdfInterpretationPoints, classDisplayName } from './siAoiReportInterpretation'
import {
  drawPdfDonutSlices,
  drawPdfTimelineXLabels,
} from './siAoiReportInfographicCharts';
import {
  buildAgHealthPieSlices,
  spectralContextFromReport,
  stageForIndexClassRow,
...
import {
  getSiAoiExportExecutiveSummaryParagraphs,
  getSiAoiExportExecutiveSummaryText,
  normalizeExecSummaryText,
} from './siAoiReportExecutiveText'
import { formatAoiAreaHaAndSqm } from './siAoiReportAreaFormat'

export { getSiAoiExportExecutiveSummaryText } from './siAoiReportExecutiveText'
...
function shortLegendLabel(labelEn: string, pct: number): string {
  const t = pdfSafeText(labelEn)
  const m = t.match(/(-?\d+\.?\d*)\s*(?:\.\.|–|-|to)\s*(-?\d+\.?\d*)/i)
  const range = m ? `${m[1]}..${m[2]}` : t.length > 10 ? `${t.slice(0, 9)}…` : t
  return `${range} ${pct.toFixed(0)}%`
}

/** Horizontal color bar + compact labels under each change-detection map tile. */
function drawHorizontalCellLegendPdf(
  doc: jsPDF,
  ...
  for (let i = 0; i < n; i++) {
    ...
    const col =
      row.colorHex ??
      (row.key === 'high' ? pal.high : row.key === 'medium' ? pal.medium : row.key === 'low' ? pal.low : '#64748b')
    const [R, G, B] = hexToRgbTriplet(col)
    doc.setFillColor(R, G, B)
    doc.rect(x + pad + i * segW, y, segW + 0.25, barH, 'F')
  }
  doc.setDrawColor(148, 163, 184)
  doc.setLineWidth(0.2)
  doc.rect(x + pad, y, innerW, barH, 'S')

  const labelY = y + barH + 6
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(4.4)
  doc.setTextColor(71, 85, 105)
  const labelSlots = Math.min(n, Math.max(3, Math.floor(innerW / 26)))
  const step = labelSlots >= n ? 1 : Math.ceil(n / labelSlots)
  for (let i = 0; i < n; i += step) {
    ...
    const col =
      ...
      (row.key === 'high' ? pal.high : row.key === 'medium' ? pal.medium : row.key === 'low' ? pal.low : '#64748b')
    const [R, G, B] = hexToRgbTriplet(col)
    const tx = x + pad + (i + 0.5) * segW * step
    doc.setFillColor(R, G, B)
    doc.roundedRect(tx - 8, labelY - 3.5, 5, 5, 0.5, 0.5, 'F')
    const lab = shortLegendLabel(row.labelEn, row.pct)
    doc.text(lab, tx - 2, labelY + 1, { align: 'center', maxWidth: segW * step + 4 })
  }
  return labelY + 10
}
...
function addChangeDetectionPageGrid(
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      ly += 10;
      const legendBlockH = opts?.report ? 24 : 0;
      const thumb = opts?.slotMapImages?.[idx];
      const imgH = Math.min(72, Math.max(32, cellH - 48 - legendBlockH));
      if (thumb && String(thumb).startsWith('data:image')) {
        const fitted = pdfEmbedPngFit(doc, thumb, x + 4, ly, cellW - 8, imgH);
        if (fitted.h > 0) ly += fitted.h + 3;
      }
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(6.5);
      doc.setTextColor(51, 65, 85);
      doc.text(
        `μ ${fmtMean(slot.stats.indexMean)} · ${fmtBound(slot.stats.indexMin)}–${fmtBound(slot.stats.indexMax)} · ${slot.stats.pixelCount} px`,
        x + 6,
        ly,
      );
      ly += 8;
      if (opts?.report) {
        drawHorizontalCellLegendPdf(doc, opts.report, x, yy + cellH - legendBlockH - 2, cellW);
      }
    }
  }
}
...
/** Draw executive paragraphs on page 1 without spilling past maxY (never adds pages). */
function pdfDrawExecSummaryParasPage1(
  doc: jsPDF,
  paragraphs: string[],
  margin: number,
  y: number,
  ...
  for (const para of paragraphs) {
    const lines = doc.splitTextToSize(pdfSafeText(para), textW);
    const lineH = 9 * bodyLh;
    const need = lines.length * lineH + 4;
    if (y + need > maxY) {
      const maxLines = Math.max(1, Math.floor((maxY - y - 4) / lineH));
      y = pdfTextBodyLines(doc, lines.slice(0, maxLines), margin, y, bodyLh) + 4;
      break;
    }
    y = pdfTextBodyLines(doc, lines, margin, y, bodyLh) + 4;
  }
  return y;
}

/** NDVI / index timeline anchored to the bottom of page 1. */
function drawPage1TimelineAtBottom(
  doc: jsPDF,
  report: SiAoiReportModel,
  margin: number,
  textW: number,
  ...
  if (report.timeSeries.length < 2) return;

  const chartH = 76;
  const headerH = 40;
  const blockH = headerH + chartH + 4;
  let ty = pageBottom - blockH;

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9.5);
  doc.setTextColor(15, 23, 42);
  doc.text(`${pdfSafeText(report.indexLabel)} timeline`, margin, ty);
  ty += 12;
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(71, 85, 105);
  ...
  ty += 6;
  drawCompactIndexTimelineInfographicPdf(doc, report, margin, ty, textW);
}
...
/** Justified body text for executive summary lines. */
function pdfDrawJustifiedParagraph(
  doc: jsPDF,
  text: string,
  x: number,
  y: number,
  ...
  for (const line of lines) {
    doc.text(String(line), x, cy, { maxWidth: maxW, align: 'justify' });
    cy += step;
  }
  return cy;
}
...
const PDF_WEEKLY_INTERP_POINT_COUNT = 3;

function drawAgriculturalInterpretationPdf(
  doc: jsPDF,
  ...
  const riskLine =
    ag.riskLevel === 'Low'
      ? 'Risk level: Low'
      : `Risk level: ${ag.riskLevel}${ag.riskCause ? ` — ${ag.riskCause}` : ''}`;
  ...
  for (const block of [ag.cropCondition, ag.yieldImpact]) {
    const lines = doc.splitTextToSize(pdfSafeText(block), textW);
    if (y + lines.length * 9 * bodyLh > pageH - margin - 40) break;
    y = pdfTextBodyLines(doc, lines, margin, y, bodyLh) + 4;
  }
  ...
  y += 10;
  doc.setFont('helvetica', 'normal');
  for (const pt of ag.insights) {
    const lines = doc.splitTextToSize(`• ${pdfSafeText(pt)}`, textW - 8);
    if (y + lines.length * 9 * bodyLh > pageH - margin - 24) break;
    y = pdfTextBodyLines(doc, lines, margin + 4, y, bodyLh) + 3;
  }

  const temporalLines = [
    formatLatestImageryDateLine(ag.latestImageryDate),
    ag.temporalInsightForecast,
  ];
  for (const pt of temporalLines) {
    const lines = doc.splitTextToSize(`• ${pdfSafeText(pt)}`, textW - 8);
    if (y + lines.length * 9 * bodyLh > pageH - margin - 24) break;
    y = pdfTextBodyLines(doc, lines, margin + 4, y, bodyLh) + 3;
  }
  ...
  y += 10;
  doc.setFont('helvetica', 'normal');
  for (const pt of ag.recommendations) {
    const lines = doc.splitTextToSize(`• ${pdfSafeText(pt)}`, textW - 8);
    if (y + lines.length * 9 * bodyLh > pageH - margin - 12) break;
    y = pdfTextBodyLines(doc, lines, margin + 4, y, bodyLh) + 3;
  }
  return y + 6;
}

function formatInterpretationTableArea(areaKm2: number): string {
...
/** Page 2: class name, spectral swatch, area, share — aligned with map legend colors. */
function drawWeeklyInterpretationTablePdf(
  doc: jsPDF,
  report: SiAoiReportModel,
  margin: number,
  startY: number,
  ...
  const body = report.tableRows.map(row => [
    pdfSafeText(classDisplayName(row.labelEn)),
    pdfSafeText(stageForReportTableRow(row, stageCtx)),
    '',
    formatInterpretationTableArea(row.areaKm2),
    `${row.pct.toFixed(1)}%`,
  ]);
  ...
  autoTable(doc, {
    ...
    tableWidth: textW,
    head: [['Class', 'Growth stage', 'Color', 'Area', 'Share %']],
    body,
    styles: {
      font: 'helvetica',
      fontSize: 7,
    ...
    columnStyles: {
      0: { cellWidth: Math.max(48, textW - 28 - 76 - 44 - 46), halign: 'left', fontSize: 6.5 },
      1: { cellWidth: 76, halign: 'left', fontSize: 7 },
      2: { cellWidth: 28, halign: 'center' },
      3: { cellWidth: 44, halign: 'right', fontSize: 7 },
      4: { cellWidth: 46, halign: 'right', fontStyle: 'bold', fontSize: 7 },
    },
    ...
    didDrawCell: (data: { section: string; column: { index: number }; row: { index: number }; cell: { x: number; y: number; width: number; height: number } }) => {
      ...
      const row = report.tableRows[data.row.index];
      if (!row) return;
      const hex =
        row.colorHex ??
        (row.key === 'high' ? pal.high : row.key === 'medium' ? pal.medium : pal.low);
      const [R, G, B] = hexToRgbTriplet(hex);
      const sw = Math.min(11, data.cell.width - 6);
      const sh = Math.min(7, data.cell.height - 4);
      const cx = data.cell.x + data.cell.width / 2;
      const cy = data.cell.y + data.cell.height / 2;
      doc.setFillColor(R, G, B);
      doc.setDrawColor(203, 213, 225);
      doc.setLineWidth(0.15);
      doc.roundedRect(cx - sw / 2, cy - sh / 2, sw, sh, 1, 1, 'FD');
    },
  });
  return (doc as { lastAutoTable: { finalY: number } }).lastAutoTable.finalY + 4;
}
...
function drawDoughnutPiePdf(
  ...
  for (const s of slices) {
    ...
    for (let k = 0; k < segs; k++) {
      ...
      doc.triangle(ox0, oy0, ox1, oy1, ix0, iy0, 'F');
      doc.triangle(ox1, oy1, ix1, iy1, ix0, iy0, 'F');
    }
    a0 += sweep;
  }
...
type PieLegendSlice = { label: string; pct: number; color: string };

function drawPieLegendInlinePdf(
  doc: jsPDF,
  slices: PieLegendSlice[],
  legendX: number,
  legendY: number,
): void {
  const rowH = 16;
  ...
  for (const sl of slices) {
    ...
    doc.setFillColor(R, G, B);
    doc.roundedRect(legendX, ly - 5, 8, 8, 1.5, 1.5, 'F');
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(30, 41, 59);
    const label = pdfSafeText(sl.label);
    doc.text(label, legendX + 14, ly + 2);
    const labelW = doc.getTextWidth(label);
    doc.setFont('helvetica', 'bold');
    doc.text(`${sl.pct.toFixed(1)}%`, legendX + 14 + labelW + 5, ly + 2);
    ly += rowH;
  }
}

function drawAgHealthPieInfographicPdf(
  doc: jsPDF,
  report: SiAoiReportModel,
  x: number,
  yTop: number,
  ...
  doc.setDrawColor(226, 232, 240);
  doc.setLineWidth(0.35);
  doc.roundedRect(x, yTop, w, blockH, 6, 6, 'S');

  const outerR = 34;
  const innerR = 19;
  const legendGap = 18;
  const legendColW = 108;
  const clusterW = outerR * 2 + legendGap + legendColW;
  const clusterX = x + Math.max(12, (w - clusterW) / 2);
  const pieCx = clusterX + outerR;
  const pieCy = yTop + blockH / 2;

  if (slices.length) {
    drawPdfDonutSlices(doc, slices, pieCx, pieCy, outerR, innerR);
  } else {
    doc.setDrawColor(203, 213, 225);
    doc.circle(pieCx, pieCy, outerR, 'S');
  }

  const legendX = pieCx + outerR + legendGap;
  const rowH = 16;
  const legendBlockH = slices.length * rowH;
  const legendY = yTop + (blockH - legendBlockH) / 2 + 4;
  drawPieLegendInlinePdf(doc, slices, legendX, legendY);

  return blockH;
}

/** Compact NDVI / index timeline for page-1 infographic row (vector, print-safe). */
function drawCompactIndexTimelineInfographicPdf(
  doc: jsPDF,
  report: SiAoiReportModel,
  x: number,
  yTop: number,
  ...
  if (pts.length < 2) {
    doc.setDrawColor(226, 232, 240);
    doc.setLineWidth(0.35);
    doc.roundedRect(x, yTop, w, h, 6, 6, 'S');
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(148, 163, 184);
    doc.text('Timeline unavailable — need at least two dates in the study period.', x + 12, yTop + h / 2 + 3);
    return h;
  }

  const accent: [number, number, number] =
    report.indexId === 'NDVI' || report.indexId === 'SAVI' || report.indexId === 'EVI'
      ? [21, 128, 61]
      : timelineAccentRgb(report.indexId);
  ...
  const innerW = Math.max(40, w - padL - padR);
  const innerY = yTop + padT;
  const innerH = Math.max(28, h - padT - padB);
  ...
  for (let g = 0; g <= 3; g++) {
    ...
    doc.line(innerX, yy, innerX + innerW, yy);
    const tickVal = vMin + t * span;
    const lab = report.indexId === 'LST' ? tickVal.toFixed(1) : tickVal.toFixed(2);
    doc.text(lab, x + 4, yy + 2);
  }
  ...
  for (let i = 0; i < pts.length; i++) {
    doc.circle(px(i), py(pts[i]!.value), 1.2, 'F');
  }
...
function appendDataInsightsPdf(doc: jsPDF, report: SiAoiReportModel, opts: SiAoiPdfExportOptions, margin: number, y0: number): number {
  ...
  doc.setFontSize(9);
  doc.setTextColor(51, 65, 85);
  const execParas = getSiAoiExportExecutiveSummaryParagraphs(report, opts.executiveSummaryAi);
  for (const para of execParas) {
    y = pdfEnsureSpace(doc, y, margin, 40);
    y = pdfDrawJustifiedParagraph(doc, para, margin, y, textW, bodyLh) + 10;
  }
  y += 6;
...
function appendLiveLayerAnalysisPdf(
  ...
  if (live.cover) {
    y = pdfEnsureSpace(doc, y, margin, 56);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9.5);
    doc.text(pdfSafeText('Cover summary (active layer)'), margin, y);
    y += 12;
    autoTable(doc, {
      startY: y,
      margin: { left: margin, right: margin },
      tableWidth: textW,
      head: [['Category', 'Share %', 'Area (ha)', 'Area (m²)']],
      body: [
        [
          live.cover.positiveLabel,
          `${live.cover.positivePct.toFixed(1)}%`,
          live.cover.positiveAreaHa.toFixed(2),
          Math.round(live.cover.positiveAreaM2).toLocaleString('en-US'),
        ],
        [
          live.cover.negativeLabel,
          `${live.cover.negativePct.toFixed(1)}%`,
          live.cover.negativeAreaHa.toFixed(2),
          Math.round(live.cover.negativeAreaM2).toLocaleString('en-US'),
        ],
      ],
      styles: { font: 'helvetica', fontSize: 8.5, cellPadding: 5 },
      headStyles: { fillColor: [21, 94, 50], textColor: 255, fontStyle: 'bold' },
  ...
  if (live.classRows.length) {
    y = pdfEnsureSpace(doc, y, margin, 80);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9.5);
    doc.text(
      pdfSafeText(`${live.activeLayerLabel} classification (${live.legendBandCount} bands)`),
      margin,
      y,
    );
    y += 12;
    const liveStageCtx = {
      ...
      ndviMean: live.indices.NDVI?.mean ?? null,
      ndmiMean: live.indices.NDMI?.mean ?? null,
      eviMean: live.indices.EVI?.mean ?? null,
    };
    autoTable(doc, {
      startY: y,
      margin: { left: margin, right: margin },
      tableWidth: textW,
      head: [['Class', 'Stage (Crop Growth Stage)', 'Share %', 'Area']],
      body: live.classRows.map(c => [
        pdfSafeText(classDisplayName(c.label)),
        pdfSafeText(stageForIndexClassRow(c, liveStageCtx)),
        `${c.pct.toFixed(1)}%`,
  ...
  if (layerIds.length) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9.5);
    doc.text(pdfSafeText(`${live.activeLayerLabel} — AOI statistics`), margin, y);
    y += 12;
...
/**
 * Scientific GIS report — exactly two pages:
 * Page 1: executive summary, class bars, health pie, NDVI timeline (anchored at bottom).
 * Page 2: AOI map + interpretation table + domain interpretation.
 */
async function buildAoiAnalysisPdfDocument(doc: jsPDF, report: SiAoiReportModel, opts: SiAoiPdfExportOptions) {
  ...
  const page1Bottom = pdfPageBottom(doc, margin) - 12;
  const timelineReserve = report.timeSeries.length >= 2 ? 122 : 0;
  const page1ContentMaxY = page1Bottom - timelineReserve;
  const pieBlockH = 118;
  ...
  const meta = [
    ...
    `Report type: ${pdfSafeText(siAoiReportTypeLabel(report.reportType))}`,
    `Period: ${report.dateStart} to ${report.dateEnd}`,
    `AOI area: ${formatAoiAreaHaAndSqm(report.aoiAreaKm2)}`,
    `Legend bands: ${report.legendBandCount}`,
    `Style mode: ${styleMode}`,
  ];
  for (const line of meta) {
    if (y + 14 > page1ContentMaxY) break;
    y = pdfDrawParagraph(doc, line, margin, y, textW, 1.15, 1);
    y += 2;
  }
  y += 6;

  if (y + 20 < page1ContentMaxY) {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.setTextColor(15, 23, 42);
    ...
    y += 14;
    const execParas = getSiAoiExportExecutiveSummaryParagraphs(report, opts.executiveSummaryAi);
    y = pdfDrawExecSummaryParasPage1(doc, execParas, margin, y, textW, page1ContentMaxY, bodyLh);
    y += 6;
  }

  const barHIdeal = 34 + report.tableRows.length * PDF_CLASS_ROW_MIN_PT;
  const spaceForBarsPie = page1ContentMaxY - y - pieBlockH - 16;
  if (spaceForBarsPie > 52) {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9.5);
    doc.setTextColor(15, 23, 42);
    doc.text('Index comparison (class bars)', margin, y);
    y += 12;
    const barH = Math.min(barHIdeal, spaceForBarsPie - 12);
    const barDrawnH = drawClassComparisonBarsPdf(
      doc,
      report.tableRows,
      report.indexLabel,
      margin,
  ...
  if (y + pieBlockH + 8 <= page1ContentMaxY) {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9.5);
    doc.setTextColor(15, 23, 42);
    doc.text('AOI health distribution', margin, y);
    y += 12;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(71, 85, 105);
    ...
    y += 6;
    const pieH = drawAgHealthPieInfographicPdf(doc, report, margin, y, textW);
    y += pieH + 8;
  }

  drawPage1TimelineAtBottom(doc, report, margin, textW, page1Bottom);

  // Page 2 only — never add pages from page-1 layout
  doc.addPage();
  y = margin;
  const tableBlockH = 22 + report.tableRows.length * 14;
  const pointsReserve = 92;
  const page2TitleH = 18;
  const mapMaxH = Math.max(
    150,
    pageH - margin * 2 - page2TitleH - tableBlockH - pointsReserve - 28,
  );

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(10);
  doc.setTextColor(15, 23, 42);
  doc.text('Map', margin, y);
  y += page2TitleH;
  let mapDrawnH = 0;
  const mapBounds =
    ...
    try {
      ...
      if (fitted.h >= 1) {
        drawPdfCartographerMapLayout(doc, fitted.x, fitted.y, fitted.w, fitted.h, mapBounds, report, {
          embeddedCartography: opts.aoiMapCartographyEmbedded === true,
        });
      }
      if (fitted.h < 1) {
        doc.setFontSize(9);
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(15, 23, 42);
  doc.text('Interpretation table', margin, y);
  y += 9;
  y = drawWeeklyInterpretationTablePdf(doc, report, margin, y, textW);
  y += 6;

  if (opts.agriculturalInterpretation) {
    y = drawAgriculturalInterpretationPdf(
      doc,
      opts.agriculturalInterpretation,
      margin,
      y,
      textW,
      pageH,
      bodyLh,
      siAoiReportTypeInterpretationSectionTitle(report.reportType),
    );
  } else {
    const points =
      opts.interpretationPoints?.filter(p => p?.trim()).slice(0, PDF_WEEKLY_INTERP_POINT_COUNT) ??
      buildWeeklyPdfInterpretationPoints(report);
    ...
    doc.setFontSize(8.5);
    doc.setTextColor(30, 41, 59);
    for (let i = 0; i < Math.min(PDF_WEEKLY_INTERP_POINT_COUNT, points.length); i++) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(30, 41, 59);
      y = pdfTextBodyLines(doc, lines, margin + 14, y, bodyLh) + 4;
    }
  }
...
function buildTimeSeriesChangeDetectionPdfDocument(doc: jsPDF, report: SiAoiReportModel, opts: SiAoiPdfExportOptions) {
  ...
  drawPdfReportBanner(
    doc,
    'Data & insights',
    `${report.indexLabel} · ${report.aoiName} · ${report.dateStart} .. ${report.dateEnd}`,
    margin,
  );
  let y = 58;
  ...
  if (report.timeSeries.length >= 2) {
    const timelineBlockH = 168;
    y = pdfEnsureSpace(doc, y, margin, timelineBlockH);
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(15, 23, 42);
    ...
    if (opts.chartImageDataUrl?.startsWith('data:image')) {
      const fitted = pdfEmbedPngFit(doc, opts.chartImageDataUrl, margin, y, chartW, 140);
      y += (fitted.h > 0 ? fitted.h : 140) + 14;
    } else {
      drawIndexTimelineVectorPdf(doc, report.timeSeries, report.indexId, margin, y, chartW, 140);
      y += 152;
    }
  }
  ...
  doc.text(`Period ${report.dateStart} .. ${report.dateEnd}   ·   AOI area ${formatAoiAreaHaAndSqm(report.aoiAreaKm2)}`, margin + 6, 50);
