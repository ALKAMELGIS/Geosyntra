...
import { waitForReactPaint } from './siMapRenderSync';
...
export type SiReportMapSnapshotRequest = {
  ...
  /** Short WMS wait for instant report preview (freezeViewport captures). */
  previewFast?: boolean;
...
async function captureCurrentCanvasFrame(
  ...
  if (isSnapshotCanvasLikelySyntheticHeatmap(canvas) === true) return null;
...
async function waitForSnapshotLayers(
  ...
  const previewFast = opts.previewFast === true;
  const requireIndex = opts.requireIndexLayer !== false;
  if (requireIndex) {
    await ensureSiMapWmsOverlayReady(map, {
      timeoutMs: previewFast
        ? 3200
        : batch
          ? 10_000
          : analysisFrame
        ...
        : batch
          ...
          : analysisFrame
            ...
            : opts.freezeViewport
              ? 14000
              : 12000,
      idlePasses: previewFast || batch ? 1 : 2,
    });
    return;
  }

  if (analysisFrame && !previewFast) {
  ...
  await waitForVisibleMapLayersReady(map, {
    timeoutMs: previewFast ? 650 : batch ? 900 : 2200,
    wmsTimeoutMs: previewFast ? 520 : batch ? 720 : 1800,
  });
}
...
async function captureExportFast(map: MapboxMap, opts: SiReportMapSnapshotRequest): Promise<string | null> {
  ...
  return runLightSnapshotLock(async () => {
    ...
    try {
      ...
      if (!opts.freezeViewport && opts.fitBounds) {
        try {
...
async function captureExportQuality(map: MapboxMap, opts: SiReportMapSnapshotRequest): Promise<string | null> {
  ...
  return runLightSnapshotLock(async () => {
    ...
    try {
      ...
      await waitForSnapshotLayers(map, opts, !opts.freezeViewport && Boolean(opts.fitBounds));
