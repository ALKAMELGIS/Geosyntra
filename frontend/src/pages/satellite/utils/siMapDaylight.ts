...
function siMapUsesStandardStyle(map: MapboxLightingMap, basemapId?: string): boolean {
  const styleUrl =
    typeof map.getStyle === 'function'
      ...
      : basemapId ?? '';
  return (
    basemapId === 'mapbox-standard-satellite' ||
    styleUrl.includes('standard') ||
    styleUrl.includes('Standard')
  );
}
...
function syncSiMapDaylightTerrainLayers(map: MapboxMap, spec: SiMapDaylightLightSpec): void {
  try {
    if (map.getLayer(HILLSHADE_LAYER_ID)) {
      ...
      const warm = spec.direction.elevationDeg > 8;
      map.setPaintProperty(
        HILLSHADE_LAYER_ID,
...
function applySiMapStandardStyleLighting(
  ...
  try {
    map.setConfigProperty('basemap', 'lightPreset', siMapStandardLightPreset(minutes));
  } catch {
...
/** Apply ArcGIS Scene–style sun lighting to Mapbox GL (3D directional lights + terrain sync). */
 ...
 * Flip to `true` for continuous loop playback (restart from 12:00 AM).
 */
export const SI_DAYLIGHT_PLAYBACK_LOOP = false;
