...
describe('runSiCropHealthAnalysis', () => {
  ...
  it('detects lower scores when NDVI is poor', () => {
    ...
    expect(poor!.summary.disease_active.count + poor!.summary.early_disease.count).toBeGreaterThan(
      healthy!.summary.disease_active.count + healthy!.summary.early_disease.count,
    );
  });
});
