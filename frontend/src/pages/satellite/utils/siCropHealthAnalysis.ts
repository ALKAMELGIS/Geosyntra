...
import {
  buildCropHealthSeverityLayers,
  buildCropHealthStressRaster,
} from './siCropHealthRasterHeatmap';
import { computeCropHealthStressModel } from './siCropHealthStressModel';
...
import type {
  ...
  SiCropHealthEnvironmentalSnapshot,
  SiCropHealthHotspot,
...
function classifyCondition(stressIndex: number): SiCropHealthCondition {
  const health = 1 - stressIndex;
  if (health >= 0.72) return 'healthy';
  if (health >= 0.52) return 'stress';
  if (health >= 0.35) return 'early_disease';
  return 'disease_active';
}

function classifySeverity(stressIndex: number): SiCropHealthSeverity {
  if (stressIndex < 0.35) return 'low';
  if (stressIndex < 0.58) return 'medium';
  return 'high';
}
...
function clusterHotspots(cells: SiCropHealthCell[]): SiCropHealthHotspot[] {
  ...
  return clusters.slice(0, 12).map((g, idx) => {
    ...
    const worst = g.reduce((w, c) => (c.stressIndex > w.stressIndex ? c : w), g[0]!);
    return {
      id: `hotspot-${idx}`,
      lng,
      lat,
...
export function runSiCropHealthAnalysis(
  ...
  input: RunSiCropHealthAnalysisInput & {
    ...
    aoiName: string;
    feature: Feature;
    dataSource?: SiCropHealthEnvironmentalSnapshot['dataSource'];
  },
  ...
  const ndviMeanAll = ndviArr.reduce((a, b) => a + (Number.isFinite(b) ? b : 0), 0) / n;
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    const localDelta = input.ndviAnalysisEnabled ? ndvi - ndviMeanAll + ndviDeltaAoi : 0;
    const ndmi = ndmiArr?.[i];
    const model = computeCropHealthStressModel({
      ndvi,
      evi,
      savi,
      ...
      useNdviTemporal: input.ndviAnalysisEnabled,
      useNdmi: Boolean(input.useSoilMoistureIndex),
      useWeather: input.aiDiseaseEnabled,
    });
    const stressIndex = model.stressIndex;
    cells.push({
      lng: pt.lng,
      lat: pt.lat,
      ndvi,
      evi,
      savi,
      ndmi: Number.isFinite(ndmi) ? Number(ndmi) : undefined,
      ndviDelta: localDelta,
      score: model.healthScore,
      stressIndex,
      condition: classifyCondition(stressIndex),
      severity: classifySeverity(stressIndex),
    });
  }
  ...
  const ndmiVals = cells.map(c => c.ndmi).filter((v): v is number => v != null && Number.isFinite(v));
  const ndmiMean = ndmiVals.length ? ndmiVals.reduce((a, b) => a + b, 0) / ndmiVals.length : null;
  const stressPct =
    ((summary.stress.count + summary.early_disease.count + summary.disease_active.count) / cells.length) *
    100;
  const trend = buildSiCropHealthTrend(input.weeklyComposites, stressPct);
  const hotspots = clusterHotspots(cells);
  const stressRaster = buildCropHealthStressRaster(cells, input.feature);
  const severityLayers = buildCropHealthSeverityLayers(cells, input.feature);

  const environmental: SiCropHealthEnvironmentalSnapshot = {
    ndviMean,
    ndmiMean,
    temperatureC: input.weather.temperatureC,
    soilMoisturePct: input.weather.soilMoisturePct,
    humidityPct: input.weather.humidityPct,
    dataSource: input.dataSource ?? 'mpc',
  };
  ...
  return {
    ...
    modelLabel: input.aiDiseaseEnabled
      ? 'Multi-index stress (NDVI·EVI·SAVI·NDMI + temperature & soil moisture)'
      : 'NDVI temporal stress model',
    ndviMean,
    ndviDeltaWeek: ndviDeltaAoi,
    weatherStress: cells.reduce((s, c) => s + c.stressIndex, 0) / cells.length,
    summary,
    cells,
    hotspots,
    trend,
