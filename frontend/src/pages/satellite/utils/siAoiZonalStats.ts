...
import { computeLivePixelStatistics, computeStatisticsHistograms } from './liveAoiSpectralStats';
...
export type SiAoiRasterPixelSample = {
  ...
  /** Server-side pixel statistics when raw value arrays are omitted. */
...
/** AOI area (+ optional active-index stats from the visible map layer when raster is unavailable). */
export function computeAoiGeometryBaseline(
  feature: GeoJSON.Feature,
  analysisDateIso: string,
  activeLayer?: {
    layerId: StaticAoiChartLayerId;
    stats: Pick<SiAoiZonalIndexStats, 'mean' | 'min' | 'max' | 'std'>;
  },
  ...
  if (activeLayer && Number.isFinite(activeLayer.stats.mean)) {
    indices[activeLayer.layerId] = {
      mean: activeLayer.stats.mean,
      min: activeLayer.stats.min,
      max: activeLayer.stats.max,
      std: activeLayer.stats.std ?? 0,
      validCount: 0,
    };
  }
  return {
    ...
    indices,
    dataSource: activeLayer ? 'map-layer' : 'geometry',
  };
}
...
function statsFromValues(vals: number[]): SiAoiZonalIndexStats | null {
  const live = computeStatisticsHistograms(vals);
  if (!live) return null;
  return {
    mean: live.mean,
    median: live.median,
...
function statsFromMpcLayerEntry(entry: MpcZonalSampleResult['layers'][string] | undefined): SiAoiZonalIndexStats | null {
  if (!entry) return null;
  const fromPixels = statsFromValues(entry.values ?? []);
  if (fromPixels) return fromPixels;
  const st = entry.statistics;
  if (!st || !Number.isFinite(st.mean)) return null;
  const histCount = st.histogram?.reduce((sum, b) => sum + (b.count ?? 0), 0) ?? 0;
  return {
    mean: st.mean,
    median: st.median,
    min: st.min,
...
export function mpcResultToRasterPixelSample(
  ...
  const mpcLayerStats: Partial<Record<StaticAoiChartLayerId, SiAoiZonalIndexStats>> = {};
  for (const id of layerIds) {
    const entry = result.layers?.[id];
    if (entry?.values?.length) layers[id] = entry.values;
    const st = statsFromMpcLayerEntry(entry);
    if (st) mpcLayerStats[id] = st;
    if (entry?.statistics?.histogram?.length) {
      histograms[id] = entry.statistics.histogram;
    }
  }
  if (!Object.keys(mpcLayerStats).length && !Object.keys(layers).length) return null;
...
function zonalAnalyticsFromRaster(
  ...
  for (const id of layerIds) {
    const masked = extractMaskedPixelValues(raster, id, feature);
    if (masked.length) {
    ...
    if (raster.layers[id]?.length) continue;
    const cached = raster.mpcLayerStats?.[id];
    if (cached) {
      indices[id] = cached;
      validPixelCount = Math.max(validPixelCount, cached.validCount);
    }
  }
...
/** Pixel values inside AOI mask — ignores NoData and points outside the polygon. */
export function extractMaskedPixelValues(
  raster: SiAoiRasterPixelSample,
  layerId: StaticAoiChartLayerId,
  feature?: GeoJSON.Feature | null,
): number[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (geom) {
      const p = grid[i]!;
      if (!pointInAoiGeometry(p.lng, p.lat, geom)) continue;
    }
    out.push(v);
  }
  return out;
...
/** Min / mean / max for live AOI popups — masked raster pixels only (no timeline metadata). */
export function liveRasterIndexStats(
  raster: SiAoiRasterPixelSample | null | undefined,
  layerId: StaticAoiChartLayerId,
  feature?: GeoJSON.Feature | null,
): Pick<SiAoiZonalIndexStats, 'mean' | 'min' | 'max' | 'validCount'> | null {
  if (!raster) return null;
  const raw = raster.layers[layerId];
  if (raw?.length) {
    const masked = extractMaskedPixelValues(raster, layerId, feature);
    const st = statsFromValues(masked);
    if (st) return { mean: st.mean, min: st.min, max: st.max, validCount: st.validCount };
    return null;
  }
  const mpc = raster.mpcLayerStats?.[layerId];
  if (mpc && Number.isFinite(mpc.mean)) {
    return {
      mean: mpc.mean,
      min: mpc.min,
      max: mpc.max,
      validCount: mpc.validCount,
    };
  }
  return null;
}
...
export function isStaticAoiChartLayerId(id: string): id is StaticAoiChartLayerId {
  return STATIC_AOI_CHART_LAYER_OPTIONS.some(o => o.id === id);
}

/** Map active WMS / index name to chart layer id (popup + zonal engine). */
export function inferStaticAoiChartLayerFromWmsName(
  layerName: string,
  ...
  const fb = (fallbackIndex ?? '').trim().toUpperCase();
  if (fb && isStaticAoiChartLayerId(fb)) return fb;
  return 'NDVI';
}

/** Active map layer for AOI popups — WMS id + picker fallback + display label. */
export function resolveActiveAoiChartLayer(opts: {
  activeWmsLayer?: string;
  selectedIndex?: string;
  wmsLayerLabel?: string;
}): { layerId: StaticAoiChartLayerId; label: string } {
  const layerId = inferStaticAoiChartLayerFromWmsName(
    opts.activeWmsLayer?.trim() || opts.selectedIndex?.trim() || '',
    opts.selectedIndex,
  );
  const meta = STATIC_AOI_CHART_LAYER_OPTIONS.find(o => o.id === layerId);
  const label = opts.wmsLayerLabel?.trim() || meta?.label || layerId;
