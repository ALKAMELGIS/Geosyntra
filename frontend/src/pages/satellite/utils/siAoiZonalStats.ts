...
/** Min / mean / max for live AOI popups — raster pixel arrays only (no timeline metadata). */
export function liveRasterIndexStats(
  raster: SiAoiRasterPixelSample | null | undefined,
  layerId: StaticAoiChartLayerId,
): Pick<SiAoiZonalIndexStats, 'mean' | 'min' | 'max'> | null {
  if (!raster) return null;
  const pixels = raster.layers[layerId];
  if (pixels?.length) {
    const st = statsFromValues(pixels.filter(Number.isFinite));
    if (st) return { mean: st.mean, min: st.min, max: st.max };
  }
  const mpc = raster.mpcLayerStats?.[layerId];
  if (mpc) return { mean: mpc.mean, min: mpc.min, max: mpc.max };
  return null;
}
