...
export function siAoiReportStyleModeInterpretationConfig(
  ...
  switch (mode) {
    ...
    default:
      return {
        task: 'Write a brief weekly interpretation for an AOI satellite report (PDF page 2).',
        rules: [
          'Output exactly 3 numbered points (1. through 3.), one short sentence each, plain English only.',
          'Point 1: dominant class and its area share; mention the next class only if it is meaningful (≥8%).',
          'Point 2: overall index condition in simple words (no formulas or sensor jargon).',
          'Point 3: one practical note — stress flag or routine monitoring; no long recommendations.',
          'Use class shares, mean index, and stress flags from the payload only.',
          'No markdown, no JSON.',
        ],
        systemInstruction:
          'You write concise weekly monitoring notes for land managers. Keep each point under 25 words.',
        pointCount: 3,
      };
  }
}
