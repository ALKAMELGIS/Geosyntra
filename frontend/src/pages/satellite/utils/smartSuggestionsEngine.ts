...
function buildSuggestedActions(ctx: SmartSuggestionsContext): SmartSuggestionItem[] {
  const layer = ctx.activeLayerLabel || ctx.activeLayerId || 'NDVI';
  const provider = ctx.satelliteProviderName || 'Sentinel Hub';
  const items: SmartSuggestionItem[] = [
    {
      ...
      title: 'Generate NDVI',
      description: `Compute vegetation index on ${provider} imagery`,
      insertText: `Generate NDVI analysis for the current AOI using ${layer} from ${provider}.`,
      actionId: 'focus-ndvi-layer',
      score: 90,
      badge: layer.includes('NDVI') ? 'Active' : undefined,
    },
    {
      id: 'action-compare-dates',
      category: 'actions',
      icon: 'fa-solid fa-calendar-days',
      title: 'Compare Dates',
      description: 'Contrast two time windows on the timeline',
      insertText: 'Compare vegetation between the earliest and latest timeline weeks in my AOI.',
    ...
    },
    {
      ...
      title: 'Detect Change',
      description: 'Highlight temporal shifts across composites',
      insertText: 'Detect significant change in the AOI between the start and end of the time series.',
      score: 82,
    },
    {
      id: 'action-report',
