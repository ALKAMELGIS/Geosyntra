...
import { siAoiReportTypeLabel } from './siAoiReportType';
import { getSiAoiExportExecutiveSummaryParagraphs } from './siAoiReportExecutiveText';
import { classDisplayName } from './siAoiReportInterpretation';
import {
...
function classificationTable(report: SiAoiReportModel): Table {
  ...
  const head = new TableRow({
    tableHeader: true,
    children: ['Class', 'Growth stage', 'Area', 'Share %'].map(
      t =>
        new TableCell({
    ...
    row =>
      new TableRow({
        children: [
          new TableCell({
            width: { size: 25, type: WidthType.PERCENTAGE },
            children: [new Paragraph({ text: classDisplayName(row.labelEn) })],
          }),
          new TableCell({
            width: { size: 25, type: WidthType.PERCENTAGE },
            children: [new Paragraph({ text: stageForReportTableRow(row, stageCtx) })],
          }),
          new TableCell({
            width: { size: 25, type: WidthType.PERCENTAGE },
            children: [new Paragraph({ text: formatReportTableAreaHa(row.areaKm2) })],
          }),
          new TableCell({
            width: { size: 25, type: WidthType.PERCENTAGE },
...
function appendixBlocks(report: SiAoiReportModel, opts: SiAoiPdfExportOptions): DocBlock[] {
  ...
  return [
    ...
    ...getSiAoiExportExecutiveSummaryParagraphs(report, opts.executiveSummaryAi).map(
      para =>
        new Paragraph({
...
function horizontalLegendLine(report: SiAoiReportModel): string {
  return report.tableRows.map(row => `${row.labelEn} ${row.pct.toFixed(1)}%`).join('  |  ');
}
...
async function changeDetectionGridTable(report: SiAoiReportModel, opts: SiAoiPdfExportOptions): Promise<Table> {
  ...
  const legendLine = horizontalLegendLine(report);
  const rows: TableRow[] = [];
  for (let r = 0; r < 4; r++) {
        ...
        new TableCell({
          children: [
            ...
            new Paragraph({
              children: [
                new TextRun(
                  `μ ${fmtMean(report.indexId, slot.stats.indexMean)} · ${slot.stats.pixelCount} px`,
                ),
              ],
            }),
            new Paragraph({
              children: [new TextRun({ text: legendLine, size: 16 })],
            }),
          ],
        }),
      );
...
async function buildAoiAnalysisDocxChildren(report: SiAoiReportModel, opts: SiAoiPdfExportOptions): Promise<DocBlock[]> {
  const out: DocBlock[] = [
    ...
    ...(report.satelliteProviderName?.trim()
      ...
      : []),
    p(`Index: ${report.indexLabel}   Report type: ${siAoiReportTypeLabel(report.reportType)}   Period: ${report.dateStart} … ${report.dateEnd}`),
    p(`AOI area: ${formatAoiAreaHaAndSqm(report.aoiAreaKm2)}`),
...
async function buildChangeDetectionDocxChildren(report: SiAoiReportModel, opts: SiAoiPdfExportOptions): Promise<DocBlock[]> {
  const out: DocBlock[] = [
    ...
    p(`Period ${report.dateStart} … ${report.dateEnd}   ·   AOI area ${formatAoiAreaHaAndSqm(report.aoiAreaKm2)}`),
