...
/** Pixel-based min / max / mean / histogram — use for live AOI popups (not layer metadata). */
export function computeStatisticsHistograms(
  values: number[],
  histogramBins = 24,
): LivePixelStatistics | null {
  return computeLivePixelStatistics(values, histogramBins);
}
...
export function opticalLayerIdsForSpectralProfile(activeLayerId: StaticAoiChartLayerId): StaticAoiChartLayerId[] {
  ...
  return STATIC_AOI_CHART_LAYER_OPTIONS.some(o => o.id === activeLayerId) ? [activeLayerId] : [];
}
