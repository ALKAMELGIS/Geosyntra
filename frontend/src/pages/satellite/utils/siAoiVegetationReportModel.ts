...
import { classRowsToReportTableRows, formatAreaTriple } from './siIndexClassAnalytics';
import { formatReportIndexValue } from './siAoiReportLiveAnalysisSnapshot';
import { buildFallbackLiveIndexExecutiveSummary } from './siAoiLiveIndexExecutiveSummary';
import {
...
export type SiAoiReportModel = {
  ...
  /** Domain template for Gemini executive summary and interpretation. */
  reportType: SiAoiReportType;
  /** Active map layers at report generation time (drives Gemini domain context). */
...
export function buildSiAoiVegetationReport(input: {
  ...
  const {
    ...
    activeLayersContext: activeLayersContextIn,
  } = input;
  const reportStyleMode = reportStyleModeIn ?? DEFAULT_SI_AOI_REPORT_STYLE_MODE;
  const reportType = reportTypeIn ?? DEFAULT_SI_AOI_REPORT_TYPE;
  ...
  return {
    ...draft,
    summaryLinesEn: [buildFallbackLiveIndexExecutiveSummary(draft)],
  };
}

/**
 * Merges AOI-clipped raster class statistics into the report model (table, summary, charts).
 * All metrics derive from the active spectral layer pixels only.
 */
export function applyLiveLayerAnalysisToReport(
  report: SiAoiReportModel,
  live: SiAoiReportLiveAnalysisSnapshot | null | undefined,
): SiAoiReportModel {
  if (!live?.classRows?.length || live.dataSource !== 'raster') {
    return { ...report, liveLayerAnalysis: live ?? report.liveLayerAnalysis ?? null };
  }

  const tableRows = classRowsToReportTableRows(live.classRows);
  const meanStr =
    live.healthPrimaryMean != null
      ? formatReportIndexValue(live.healthPrimaryMean, live.activeLayerId)
      : live.classAnalytics?.mean != null
        ? formatReportIndexValue(live.classAnalytics.mean, live.activeLayerId)
        : '—';

  const pctSummary = tableRows.map(r => `${r.pct.toFixed(1)}%`).join(' · ');
  const analysisEn = `${live.activeLayerLabel}: raster mean ${meanStr}. Class distribution (${live.legendBandCount ?? report.legendBandCount}-band ramp): ${pctSummary}.`;

  const pieSlices = live.classRows.map(c => ({
    label: c.condition,
    pct: c.pct,
    color: c.colorHex,
  }));

  const dataInsights: SiAoiDataInsightsBundle = {
    ...report.dataInsights,
    pieSlices,
    barSeries: live.classRows.map(c => ({
      id: `c${c.classId}`,
      label: c.condition.slice(0, 18),
      valueNorm: Math.max(0, Math.min(1, c.pct / 100)),
    })),
    indexRows: report.dataInsights.indexRows,
  };

  const merged: SiAoiReportModel = {
    ...report,
    indexId: live.activeLayerId,
    indexLabel: live.activeLayerLabel,
    aoiAreaKm2: live.areaHa / 100,
    tableRows,
    analysisEn,
    dataInsights,
    liveLayerAnalysis: live,
  };
  const summaryLinesEn = [buildFallbackLiveIndexExecutiveSummary(merged)];
...
export type SiAoiPdfExportOptions = {
  ...
  executiveSummaryAi?: string | null;
  /** Legacy flat bullets (optional). */
  interpretationPoints?: string[] | null;
  /** Yield Insight — agronomic interpretation block for PDF page 2. */
  agriculturalInterpretation?: import('./siAoiAgriculturalInterpretation').SiAoiAgriculturalInterpretation | null;
  /** Map extent for scale bar (defaults from AOI outline). */
  aoiMapLngLatBounds?: SiPdfLngLatBounds | null;
  /** PNG already includes north arrow, scale bar, and legend strip. */
  aoiMapCartographyEmbedded?: boolean;
