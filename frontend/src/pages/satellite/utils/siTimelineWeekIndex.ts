import { addTimelineInterval, dateToTimelineIso, timelineDateFromIso } from './siTimelineDate';
import type {
  SiTimeSliderMode,
...
/** Nearest stop index for scrub / step / playback. */
export function pickTimelineStopIdx(stops: readonly string[], isoRaw: string): number {
  ...
  const focusMs = timelineDateFromIso(iso).getTime();
  let best = 0;
  let bestDist = Infinity;
  for (let i = 0; i < stops.length; i++) {
    const dist = Math.abs(timelineDateFromIso(stops[i]!).getTime() - focusMs);
    if (dist < bestDist) {
      bestDist = dist;
      best = i;
    }
