...
import { waitForReactPaint, waitForSiMapWmsRunsSettled } from './siMapRenderSync';
import { raiseSiMapWmsRasterLayersToTop, refreshSiMapWmsRasterPaint } from './siMapWmsRasterLayerStack';
...
/** Synthetic client-side heatmap fallback (#0f172a canvas + grid cells) — never use in reports. */
export function isSnapshotCanvasLikelySyntheticHeatmap(canvas: HTMLCanvasElement): boolean | null {
  try {
    const ctx = canvas.getContext('2d', { willReadFrequently: true });
    if (!ctx) return null;
    const w = canvas.width;
    ...
    const isSlab = (s: RgbSample) => s.r <= 22 && s.g <= 32 && s.b <= 52;
    if (corners.filter(isSlab).length < 3) return false;
    let centerHits = 0;
    for (let row = 0; row < 5; row++) {
      for (let col = 0; col < 5; col++) {
        const x = Math.floor(w * (0.25 + (col / 4) * 0.5));
        const y = Math.floor(h * (0.25 + (row / 4) * 0.5));
        const d = ctx.getImageData(x, y, 1, 1).data;
        const luma = 0.299 * d[0]! + 0.587 * d[1]! + 0.114 * d[2]!;
        if (luma > 28 && luma < 220) centerHits++;
      }
    }
    return centerHits >= 8;
  } catch {
...
export async function isSnapshotPngLikelySyntheticHeatmap(pngDataUrl: string): Promise<boolean> {
  return new Promise(resolve => {
    const img = new Image();
    img.onload = () => {
      ...
      resolve(isSnapshotCanvasLikelySyntheticHeatmap(c) === true);
    };
...
/** Active Sentinel Hub WMS stack on the main map (sources + layers). */
export function listSiMapWmsStackIds(map: MapboxMap): { sourceIds: string[]; layerIds: string[] } {
  return {
    layerIds: listIndexRasterLayerIds(map),
    sourceIds: listStyleSourceIds(map).filter(isSiMapIndexRasterSourceId),
  };
}
...
/** After React toggles WMS visibility, wait for raster sources/layers to paint on the map canvas. */
export async function ensureSiMapWmsOverlayReady(
  map: MapboxMap,
  ...
  const { sourceIds, layerIds } = listSiMapWmsStackIds(map);
  if (!sourceIds.length) return;
  await waitForSiMapWmsRunsSettled(map, sourceIds, layerIds, {
...
/** Red / yellow stress bands from classified NDVI-style ramps (not desert soil). */
function isClassifiedIndexStressPixel(r: number, g: number, b: number): boolean {
  if (r > 165 && g < 135 && b < 135) return true;
  if (r > 205 && g > 168 && b < 115) return true;
  return false;
}
...
function isClassifiedIndexPixel(r: number, g: number, b: number): boolean {
  ...
  if (isClassifiedIndexStressPixel(r, g, b)) return true;
  // Synthetic ramp green (lime), not natural satellite crop green.
  if (g > 165 && r < 115 && b < 90 && g - Math.max(r, b) > 42) return true;
  const max = Math.max(r, g, b);
  const min = Math.min(r, g, b);
  return max - min >= 58 && luma >= 32 && (r > 140 || g > 140);
}
...
export function indexOverlayLikelyFromSamples(samples: readonly RgbSample[]): boolean {
  ...
  for (const { r, g, b } of samples) {
    ...
    if (luma < 18) continue;
    lumaHits++;
    if (isClassifiedIndexStressPixel(r, g, b)) stressHits++;
    if (isClassifiedIndexPixel(r, g, b)) rampHits++;
  }
  if (lumaHits < 6) return false;
  if (stressHits >= 2 && rampHits >= 5) return true;
  return rampHits >= 10;
}
...
export type SiLiveMapSnapshotOptions = {
  ...
  /** Short WMS wait for instant freezeViewport preview captures. */
  previewFast?: boolean;
};
