...
/** @deprecated Use layerIndexRows — kept for quick string fallbacks in credits. */
export function siMapPrintLayerLinesFromIndex(rows: SiMapPrintLayerIndexRow[]): string[] {
  return rows.map(r => {
    const vis = r.visible ? 'on' : 'off';
    return r.detail ? `${r.label} (${vis}) — ${r.detail}` : `${r.label} (${vis})`;
  });
}
