...
export function SiAoiReportModal({
  ...
  const prefetchAnalysisSnapshot = useCallback(
    async (built: SiAoiReportModel) => {
      ...
      setAnalysisLiveSnapshot(null);
      try {
        const bbox = siAoiReportFeatureBBoxLngLat(aoiFeat);
        const fitBounds = bbox
          ? ([
              [bbox[0], bbox[1]],
              [bbox[2], bbox[3]],
            ] as [[number, number], [number, number]])
          : changeFitBounds ?? undefined;
    ...
    [captureLiveMapSnapshot, changeFitBounds],
  );
      ...
      if (mode === 'AOI_ANALYSIS') {
        if (format === 'pdf' && liveMapCaptureOk && captureLiveMapSnapshot) {
          const rawPdf = await captureLiveMapSnapshot({
            ...
            freezeViewport: true,
            maskToAoi: false,
            fitBounds: changeFitBounds ?? undefined,
            suppressModalChrome: true,
            requireIndexLayer: true,
            ...liveSnapshotAoiOpts,
          });
              ...
              {reportView === 'change' ? (
                ...
                <>
                  ...
                  <div className="si-aoi-report-card">
                    ...
                    <p className="si-aoi-report-analysis si-aoi-report-analysis--compact">
                      Cartographer layout: live basemap + index layer (Sentinel Hub WMS), vector north arrow,
                      scale bar, and classification legend aligned to the AOI extent.
                    </p>
                    {liveMapCaptureOk ? (
