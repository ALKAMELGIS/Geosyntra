        ...
        >
            ...
            <div className="si-crop-health-panel__head-row">
              <div className="si-crop-health-panel__brand">
                ...
                <div>
                  ...
                  <p className="si-crop-health-panel__sub">
                    Sentinel-2 raster stress · NDVI · NDMI · weather fusion
                  </p>
                </div>
              </div>
          ...
          {!minimized ? (
            <div className="si-crop-health-panel__body">
              ...
              <div className="si-crop-health-panel__field">
                <label htmlFor="si-chi-sat">Satellite (STAC)</label>
                <select
                  id="si-chi-sat"
                  value={settings.satelliteCollection}
                  onChange={e => patch({ satelliteCollection: e.target.value })}
                >
                  {SI_CROP_HEALTH_SATELLITE_OPTIONS.map(s => (
              ...
              <div className="si-crop-health-panel__toggles">
                <div className="si-crop-health-panel__toggle">
                  <label htmlFor="si-chi-raster">Stress heatmap (raster)</label>
                  <input
                    id="si-chi-raster"
                    type="checkbox"
                    checked={settings.showStressRaster}
                    onChange={e => patch({ showStressRaster: e.target.checked })}
                  />
                </div>
                <div className="si-crop-health-panel__toggle">
                  <label htmlFor="si-chi-sev">Stress classes (low / med / high)</label>
                  <input
                    id="si-chi-sev"
                    type="checkbox"
                    checked={settings.showSeverityLayers}
                    onChange={e => patch({ showSeverityLayers: e.target.checked })}
                  />
                </div>
                <div className="si-crop-health-panel__toggle">
              ...
              {result ? (
                <div className="si-crop-health-panel__results">
                  <p style={{ fontSize: '0.72rem', color: '#94a3b8', margin: 0 }}>
                    {result.modelLabel} · {result.cellCount} pixels · {result.areaHa.toFixed(1)} ha
                    {result.stressRaster?.resolutionM
                      ? ` · ~${result.stressRaster.resolutionM} m`
                      : ''}
                  </p>
                  <p style={{ fontSize: '0.68rem', color: '#64748b', margin: '4px 0 0' }}>
                    NDVI {result.environmental.ndviMean.toFixed(2)}
                    {result.environmental.ndmiMean != null
                      ? ` · NDMI ${result.environmental.ndmiMean.toFixed(2)}`
                      : ''}
                    {' · '}
                    {result.environmental.temperatureC.toFixed(0)}°C · soil{' '}
                    {result.environmental.soilMoisturePct.toFixed(0)}%
                  </p>
                  ...
                  {result.stressRaster ? (
                    <button
                      type="button"
                      className="si-crop-health-panel__run"
                      style={{ marginTop: 8, background: '#1e3a5f' }}
                      onClick={() => downloadCropHealthRasterPng(result.stressRaster!)}
                    >
                      Export stress raster (PNG)
                    </button>
                  ) : null}
                </div>
              ) : null}
