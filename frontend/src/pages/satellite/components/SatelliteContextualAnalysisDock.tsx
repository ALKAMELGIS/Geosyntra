...
export function SatelliteContextualAnalysisDock(props: SatelliteContextualAnalysisDockProps) {
  ...
  const appThemeLight = settings.themeMode === 'light';
  const {
    variant,
    className = '',
    mapTool,
  ...
  const popupsToolLockTitle =
    language === 'ar'
      ? 'أغلق نوافذ Pop-up أولاً'
      : 'Close feature pop-ups before starting another map tool';
  ...
  const [panelWidth, setPanelWidth] = useState(() => {
    ...
    return variant === 'embedded' ? Math.min(300, typeof window !== 'undefined' ? window.innerWidth - 48 : 300) : 300;
  });
  ...
  useEffect(() => {
    ...
    try {
      const stored = localStorage.getItem('si-sat-ctx-surface');
      setSurface(stored === 'light' ? 'light' : 'dark');
    } catch {
      setSurface('dark');
    }
  }, [appThemeLight]);
  ...
  /** Defined alias — Flood rail and legacy snippets must not reference an undeclared identifier. */
  const showRailLabels = railWide;
  ...
  return (
    ...
    <div className={rootClass} role="presentation" dir={direction}>
      ...
      >
        ...
        {isMap && onToggleRouteMap ? (
          <button
            ...
            className={
              ...
              (mapAnalysisToolsLockedByPopups && !routeMapOpen ? ' si-sat-ctx-rail-btn--locked' : '')
            }
            title={
              mapAnalysisToolsLockedByPopups && !routeMapOpen
                ? popupsToolLockTitle
                : language === 'ar'
                  ? routeMapOpen
                    ? 'إغلاق خريطة المسار'
            ...
            disabled={mapAnalysisToolsLockedByPopups && !routeMapOpen}
        ...
        {isMap && onToggleElevProfile ? (
          <button
            type="button"
            className={
              'si-sat-ctx-rail-btn si-sat-ctx-rail-btn--map si-sat-ctx-rail-btn--elev-profile-tool' +
              (railWide ? ' si-sat-ctx-rail-btn--row si-sat-ctx-rail-btn--map-expanded' : '') +
              (isMap && !railWide ? ' si-sat-ctx-rail-btn--map-collapsed' : '') +
              (elevProfileOpen ? ' si-sat-ctx-rail-btn--active' : '') +
              (mapAnalysisToolsLockedByPopups && !elevProfileOpen ? ' si-sat-ctx-rail-btn--locked' : '')
            }
            title={
              mapAnalysisToolsLockedByPopups && !elevProfileOpen
                ? popupsToolLockTitle
                : language === 'ar'
                  ? elevProfileOpen
                    ? 'إغلاق ملف الارتفاع'
                    : 'ملف الارتفاع — رسم خط على الخريطة'
                  : elevProfileOpen
                    ? 'Close elevation profile'
                    : 'Elevation profile — draw a line on the map'
            }
            aria-label={language === 'ar' ? 'ملف الارتفاع' : 'Elevation profile'}
            aria-pressed={elevProfileOpen}
            disabled={mapAnalysisToolsLockedByPopups && !elevProfileOpen}
            onClick={() => onToggleElevProfile()}
          >
            <span className="si-sat-ctx-rail-elev-profile-glyph" aria-hidden>
              <i className="fa-solid fa-chart-area" />
            </span>
            {isMap ? (
              <span className="si-sat-ctx-rail-label" aria-hidden={!railWide}>
                <span className="si-sat-ctx-rail-label-title">{language === 'ar' ? 'ارتفاع' : 'Elev profile'}</span>
                <span className="si-sat-ctx-rail-label-desc">
                  {language === 'ar' ? 'رسم وتحليل' : 'Terrain cross-section'}
                </span>
              </span>
            ) : null}
          </button>
        ...
        {isMap && onToggleMapWeatherIntel ? (
          <button
            ...
            className={
              ...
              (mapAnalysisToolsLockedByPopups && !mapWeatherIntelActive ? ' si-sat-ctx-rail-btn--locked' : '')
            }
            title={
              mapAnalysisToolsLockedByPopups && !mapWeatherIntelActive
                ? popupsToolLockTitle
                : language === 'ar'
            ...
            disabled={mapAnalysisToolsLockedByPopups && !mapWeatherIntelActive}
        ...
        {!isMap ? (
          <div className="si-sat-ctx-rail__footer si-sat-ctx-rail-footer">
            <button
              type="button"
              className={
                'si-sat-ctx-rail-collapse' + (railWide ? ' si-sat-ctx-rail-collapse--labeled' : '')
              }
              title={
                railWide
              ...
              aria-label={
                railWide ? 'Collapse sidebar and close panel' : 'Expand toolbox labels'
              }
            ...
            >
              <span className="si-sat-ctx-rail-collapse__icon-wrap" aria-hidden>
                <i className={railWide ? 'fa-solid fa-bars' : 'fa-solid fa-angles-left'} />
              </span>
            </button>
          </div>
        ) : null}
              ...
              <header className="si-sat-ctx-panel-header">
                ...
                <div className="si-sat-ctx-panel-header-actions">
                  <button
                    ...
                    title={
                      appThemeLight
                        ? 'Panel theme follows app light mode'
                        : surface === 'dark'
                          ? 'Light surface'
                          : 'Dark surface'
                    }
                    aria-label="Toggle panel theme"
                    disabled={appThemeLight}
                ...
                {innerTab === 'main' ? (
                  <>
                    ...
                    {activeId === 'weather' && (
                      <div className="si-sat-ctx-prose">
                        <p>
                          <strong>Weather tools</strong> — Open-Meteo point forecasts and scene weather visualization over your AOI.
                        </p>
                        <p className="si-sat-ctx-muted">
                          Open Weather visualization to adjust presets, cloud cover, precipitation, fog, Sun &amp; Sky analysis, and scene slides.
                        </p>
                      </div>
                    )}
                    {activeId === 'raster' && (
