...
import { timelineDateFromIso } from '../utils/siTimelineDate';
import { pickTimelineStopIdx } from '../utils/siTimelineWeekIndex';
import {
...
function isoToRailFraction(iso: string, rangeStart: string, rangeEnd: string): number {
  ...
  if (start === end) return 0;
  const a = timelineDateFromIso(start).getTime();
  const b = timelineDateFromIso(end).getTime();
  const t = timelineDateFromIso(iso.slice(0, 10)).getTime();
  const span = b - a;
  if (span <= 0) return 0;
...
export function SatelliteMapAnalysisChrome(props: SatelliteMapAnalysisChromeProps) {
  ...
  const snapRailToClientX = useCallback(
    (clientX: number) => {
      if (!onPickTimelineStop || !railStops.length) return;
      const track = timelineRailRef.current;
      if (!track) return;
      const rect = track.getBoundingClientRect();
      if (rect.width <= 0) return;
      const ratio = Math.max(0, Math.min(1, (clientX - rect.left) / rect.width));
      const startMs = timelineDateFromIso(seriesStartIso || railStops[0]!).getTime();
      const endMs = timelineDateFromIso(seriesEndIso || railStops[railStops.length - 1]!).getTime();
      const targetMs = startMs + ratio * Math.max(0, endMs - startMs);
      let best = railStops[0]!;
      let bestDist = Infinity;
      for (const stop of railStops) {
        const dist = Math.abs(timelineDateFromIso(stop).getTime() - targetMs);
        if (dist < bestDist) {
          bestDist = dist;
          best = stop;
        }
    ...
    [onPickTimelineStop, railStops, seriesStartIso, seriesEndIso],
  );

  const onRailPointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      if (!onPickTimelineStop) return;
      e.preventDefault();
      e.stopPropagation();
      ...
      snapRailToClientX(e.clientX);
      const onMove = (ev: PointerEvent) => snapRailToClientX(ev.clientX);
    ...
    [onPickTimelineStop, snapRailToClientX],
  );
