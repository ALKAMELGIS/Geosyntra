...
export function SiAoiWorkspaceSettingsPanel({
  ...
  const summary = `${formatAreaShort(areaHa)} · ${formatRangeLabel(dateStart, dateEnd)} · ${layersOnCount}/${layersTotal} layers`
        ...
        {open ? (
          ...
          >
            <div className="si-rs-aoi-settings__panel-inner">
              ...
              <div className="si-rs-aoi-settings__layers">{layersSlot}</div>
            </div>
          </motion.div>
        ) : null}
      </AnimatePresence>
