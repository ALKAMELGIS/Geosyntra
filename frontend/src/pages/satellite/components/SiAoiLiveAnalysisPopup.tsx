...
export function SiAoiLiveAnalysisPopup({
  ...
  const blocked = status === 'error' || status === 'unavailable';
  ...
  if (blocked && !isRaster) {
    return (
      <div className="si-live-popup__state si-live-popup__state--error" role="alert">
        <p>{error || 'Real pixel analysis unavailable. Configure VITE_ANALYSIS_ENGINE_URL and ensure the analysis engine is running.'}</p>
        <p className="si-live-popup__status-hint">No mock or symbology values are shown in this panel.</p>
      </div>
    );
  }
  ...
  return (
    <div className="si-live-popup">
      ...
      <PopupSection id="confidence" title="Confidence" defaultOpen={false}>
        ...
        />
        <MetricRow label="Data source" value={isRaster ? 'AOI-clipped STAC raster' : '—'} />
