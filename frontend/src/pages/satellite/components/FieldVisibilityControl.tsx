...
type Props = {
  ...
  /** Extra classes on the trigger button (e.g. AGOL table icon bar). */
...
/** Must sit above `.si-layer-action-modal-overlay` (12500) and layer menus (12850). */
const FIELDVIS_POPOVER_Z = 12950
  ...
  return (
    <>
      <button
        ref={btnRef}
        type="button"
        className={['gis-fieldvis-trigger', open ? 'gis-fieldvis-trigger--open' : '', triggerClassName]
          .filter(Boolean)
          .join(' ')}
        aria-label="Field visibility"
        ...
        onClick={() => setOpen(v => !v)}
      >
        <i className={`fa-solid ${triggerIcon}`} aria-hidden="true" />
      </button>
