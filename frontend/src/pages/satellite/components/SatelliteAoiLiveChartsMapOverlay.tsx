...
import { SiLiveAoiSpectralStatsCard } from './SiLiveAoiSpectralStatsCard';
...
import { formatActiveIndexStat } from '../utils/liveAoiEnvironmentalIndicators';
...
export function SatelliteAoiLiveChartsMapOverlay({
  ...
  const hasPartialLiveData = Boolean(
    snapshot?.zonal?.areaHa ||
      snapshot?.activeIndexStats ||
      (snapshot?.primaryIndexValue != null && Number.isFinite(snapshot.primaryIndexValue)) ||
      barRows.length > 0,
  );
  const showEngineWarning =
    (status === 'error' || status === 'unavailable') && hasPartialLiveData;
  const showBlockingError =
    (status === 'error' || status === 'unavailable' || (snapshot != null && snapshot.dataSource !== 'raster' && barRows.length === 0)) &&
    !hasPartialLiveData;
  return (
    <div
      ref={panelRef}
    ...
    >
      <div className="si-map-analysis-charts-head" onPointerDown={onChartsHeadPointerDown}>
        <div className="si-map-analysis-charts-head-text">
          ...
          <div className="si-map-analysis-charts-subtitle">
            {indexLabel} · current map layer · live raster only (use Stop Timeline to return here)
      ...
      {status !== 'loading' && showBlockingError ? (
        <p className="si-map-analysis-charts-subtitle si-map-analysis-charts-subtitle--pad si-live-aoi-error">
          {error ||
            'Connect the analysis engine and ensure Sentinel-2 coverage for this AOI and date range.'}
        </p>
      ) : null}
      {status !== 'loading' && showEngineWarning ? (
        <p className="si-map-analysis-charts-subtitle si-map-analysis-charts-subtitle--pad si-live-aoi-warn">
          {error
            ? `${error} — showing AOI geometry and visible map layer values until pixel sampling succeeds.`
            : 'Pixel sampling unavailable — showing AOI area and map layer values.'}
        </p>
      ) : null}
        ...
        <>
          ...
          <div className="si-live-aoi-metrics">
            <div className="si-live-aoi-metric-card">
              <div className="si-live-aoi-metric-k">Imagery date</div>
              <div className="si-live-aoi-metric-v">{snapshot.analysisDateIso}</div>
            </div>
            {snapshot.seriesEndIso ? (
              <div className="si-live-aoi-metric-card">
                <div className="si-live-aoi-metric-k">End date</div>
                <div
                  className="si-live-aoi-metric-v"
                  title={
                    snapshot.seriesStartIso
                      ? `${snapshot.seriesStartIso} → ${snapshot.seriesEndIso}`
                      : snapshot.seriesEndIso
                  }
                >
                  {snapshot.seriesEndIso}
          ...
          <div className="si-map-analysis-charts-grid si-map-analysis-charts-grid--below">
            ...
            <div className="si-map-analysis-chart-card">
              <div className="si-map-analysis-chart-kicker">Environmental indicators (live)</div>
            ...
            {snapshot.dataSource === 'raster' && barRows.length > 0 ? (
