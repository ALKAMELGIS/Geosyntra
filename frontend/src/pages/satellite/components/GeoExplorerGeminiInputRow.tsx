...
import {
  GeoCartAiComposerPanel,
  type GeoCartAiComposerContext,
} from './GeoCartAiComposerPanel'
...
export function GeoExplorerGeminiInputRow(props: GeoExplorerGeminiInputRowProps) {
  ...
  const geoCartAnchorRef = useRef<HTMLDivElement | null>(null)
  const [composerFocused, setComposerFocused] = useState(false)
  const [smartPanelOpen, setSmartPanelOpen] = useState(false)
  const [geoCartOpen, setGeoCartOpen] = useState(false)
            ...
            <div className={pfx(cssPrefix, 'composer-voice-hint')} aria-live="polite">
              {voiceUiState === 'capturing' && interimPreview ? (
                ...
                <div className="geo-chat-composer-tools" aria-label="Composer tools">
                  ...
                  {geoCartAiContext ? (
                    <div ref={geoCartAnchorRef} className="geo-cart-ai-anchor">
                      <button
                        type="button"
                        className={`${pfx(cssPrefix, 'composer-icon-btn')} geo-cart-ai-trigger${geoCartOpen ? ' geo-cart-ai-trigger--open' : ''}`}
                        onClick={() => setGeoCartOpen(o => !o)}
                        disabled={busy}
                        aria-expanded={geoCartOpen}
                        aria-haspopup="dialog"
                        aria-label={geoCartOpen ? 'Close Geo-Cart AI' : 'Geo-Cart AI spatial extraction'}
                        title="Geo-Cart AI · Deep learning feature extraction"
                      >
                        <i className="fa-solid fa-crosshairs" aria-hidden />
                      </button>
                      <GeoCartAiComposerPanel
                        open={geoCartOpen}
                        onClose={() => setGeoCartOpen(false)}
                        context={geoCartAiContext}
                      />
                    </div>
                  ) : null}
                </div>
