...
} from '../utils/siAoiReportExecutiveText';
...
export type SiAoiReportDataInsightsSectionProps = {
  ...
  /** Optional timeline slot (PDF page 1 uses the compact chart in SiAoiReportInfographicPanel). */
...
export function SiAoiReportDataInsightsSection({
  ...
  const execAi = geminiLoading ? null : geminiSummary;
  const execDisplay = getSiAoiExportExecutiveSummaryText(report, execAi);
  const execParagraphs = getSiAoiExportExecutiveSummaryParagraphs(report, execAi);
    ...
    <div className="si-aoi-insights">
      <div className="si-aoi-insights__header">
        ...
        <p className="si-aoi-insights__sub">
          Live Index executive summary (five lines), multi-index statistics, and vector mini-charts. Values follow
          AOI-clipped raster pixels when live analysis is available.
        </p>
      </div>
      ...
      <section className="si-aoi-insights__block" aria-labelledby="si-aoi-insights-exec">
        ...
        <div className="si-aoi-insights__exec-block">
          {execParagraphs.map((para, i) => (
            <p key={i} className="si-aoi-insights__exec">
              {para}
            </p>
          ))}
        </div>
      ...
      {timelineSlot ? (
        <section className="si-aoi-insights__block si-aoi-insights__block--timeline" aria-labelledby="si-aoi-insights-timeline">
          <h4 id="si-aoi-insights-timeline" className="si-aoi-insights__block-title">
            {report.indexLabel} timeline
          </h4>
          {timelineSlot}
        </section>
      ) : null}
