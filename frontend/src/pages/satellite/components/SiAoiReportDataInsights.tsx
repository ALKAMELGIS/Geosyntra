...
export type SiAoiReportDataInsightsSectionProps = {
  ...
  /** NDVI / index timeline chart — rendered at the end of page 1 (matches PDF order). */
...
export function SiAoiReportDataInsightsSection({
  ...
  const execDisplay = geminiLoading
    ? getSiAoiExportExecutiveSummaryText(report, null)
    : getSiAoiExportExecutiveSummaryText(report, geminiSummary);
  const execParagraphs = executiveSummaryParagraphs(execDisplay, 4);
