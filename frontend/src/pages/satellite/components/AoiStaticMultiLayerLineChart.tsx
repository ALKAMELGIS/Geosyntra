...
export function AoiStaticMultiLayerLineChart({
  ...
  const scatterIndexCross = useMemo(() => {
    ...
    const olsAnnotationLines = formatOlsAnnotationLines({
      yLabel: model.yLabel,
      xLabel: model.xLabel,
      slope: model.slope,
      intercept: model.intercept,
    ...
    const srcLabel = model.dataSource === 'raster' ? 'MPC raster pixels' : 'AOI grid cells';
      ...
      plugins: {
        title: {
          ...
          text: [
            ...
            `${model.weekLabel ? `${model.weekLabel} · ` : ''}${srcLabel} · n = ${model.n}`,
          ],
          color: titleColor,
