...
function defaultPanelPos(): PanelPos {
  return siMapLeftPopoutFixedPosition('weather', 340);
}
...
export type SiMapWeatherToolPanelProps = {
  ...
  onSunSkySettingsChange: Dispatch<SetStateAction<SiMapSunSkySettings>>;
  mapCenter: { lng: number; lat: number };
  sunSkyLosSketchMode: 'observer' | 'target' | null;
  onSunSkyLosSketchModeChange: (mode: 'observer' | 'target' | null) => void;
  onClearSunSkyLos: () => void;
};
...
export function SiMapWeatherToolPanel({
  ...
  const measure = useCallback(() => {
    const el = shellRef.current;
    if (!el) return { width: 264, height: 340 };
    const r = el.getBoundingClientRect();
    return { width: r.width || 264, height: r.height || 340 };
  }, []);
            ...
            <div className="si-weather-panel__body">
              ...
              {isSunSkyPanel ? (
                <SiMapSunSkyWeatherBody
                  ...
                  sunSkySettings={sunSkySettings}
                  onSunSkySettingsChange={onSunSkySettingsChange}
                  mapCenter={mapCenter}
                  isLightTheme={isLightPanel}
                  losSketchMode={sunSkyLosSketchMode}
                    ...
                    {slidesStore.slides.map(sl => (
                      <li key={sl.id}>
                        ...
                        >
                          ...
                          <span>
                            {siMapWeatherActivePresets(sl.weather).join(', ')}
                          </span>
                        </button>
                        <button
                          type="button"
