...
export function ProfileCover({
  ...
  const onPointerDown = useCallback(
    (e: React.PointerEvent) => {
      if (!coverUrl || !bannerRef.current) return
      e.preventDefault()
      setDragging(true)
    ...
    [coverUrl, onPositionChange],
  )

  return (
    <motion.div
      ref={bannerRef}
      className="group relative h-[min(42vw,320px)] min-h-[220px] max-h-[380px] w-full overflow-hidden rounded-2xl border border-white/10 shadow-2xl shadow-black/50 sm:rounded-3xl"
    ...
    >
        ...
        {coverUrl ? (
          <img
            ...
            className={cn(
              ...
              dragging ? 'cursor-grabbing' : coverUrl ? 'cursor-grab' : '',
            )}
      ...
      <motion.div
        className={cn(
          'absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/50 backdrop-blur-sm transition-opacity duration-300',
      ...
      >
        ...
        <CoverUploader hasCover={Boolean(coverUrl)} saving={saving} onPick={onPick} onRemove={onRemove} />
      </motion.div>
    </motion.div>
  )
}
