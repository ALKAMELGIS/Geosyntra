...
import { homeWizardSearch, navigateToHomeWizard } from '../../lib/homeWizardEntry'
...
import { ProfileTechBackdrop } from './components/ProfileTechBackdrop'
...
export default function ProfilePage() {
  ...
  const onUpgradePlan = () => {
    navigateToHomeWizard(navigate, { wizard: 'pricing', authMode: 'signin', upgrade: true })
  }
  ...
  const tabButton = (t: (typeof TABS)[number]) => {
    ...
    return (
      <button
        ...
        className={cn('profile-nav__item', active && 'profile-nav__item--active')}
      >
        <Icon className="profile-nav__item-icon" aria-hidden />
        {t.label}
      </button>
    )
  }
    ...
    <ProfileShell>
      ...
      <div className="mt-8 flex flex-col gap-6 lg:flex-row lg:gap-8">
          ...
          <div className="lg:sticky lg:top-24">
            <div className="lg:hidden">
              <button
                ...
                className="profile-nav__mobile-toggle"
              >
                {TABS.find(t => t.id === tab)?.label}
                <ChevronDown className={cn('h-4 w-4 transition', mobileNavOpen && 'rotate-180')} />
              </button>
              {mobileNavOpen ? (
                <nav className="profile-nav__mobile-panel profile-nav__list" aria-label="Profile sections">
                  {TABS.map(tabButton)}
                </nav>
              ) : null}
            </div>
            <nav className="profile-nav__list hidden lg:flex" aria-label="Profile sections">
              {TABS.map(tabButton)}
            </nav>
            <button
              type="button"
              onClick={onLogout}
              className="profile-nav__item profile-nav__signout hidden lg:flex"
            >
              <LogOut className="profile-nav__item-icon" aria-hidden />
              Sign out
            </button>
          </div>
        </aside>
        ...
        <div className="min-w-0 flex-1 transition-opacity duration-300">
          ...
          {tab === 'billing' ? <ProfileBillingSection /> : null}
          ...
          {tab === 'settings' ? (
            <section className="space-y-6">
              ...
              <button type="button" onClick={onLogout} className="profile-nav__mobile-signout lg:hidden">
...
function ProfileShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-[#07080c] text-zinc-100">
      <ProfileTechBackdrop />
      <div className="relative z-[1] border-b border-white/[0.08] bg-[#07080c]/72 backdrop-blur-2xl">
      ...
      <main className="relative z-[1] mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">{children}</main>
    </div>
  )
}
