                    ...
                    onClick={() => {
                      void (async () => {
                        const next = await appPrompt(
                          `Password for ${user.email} (minimum 8 characters). This enables sign-in on all devices.`,
                          '',
                          { title: 'Enable sign-in', confirmLabel: 'Apply' },
                        )
                        if (!next) return
                        setRepairBusy(true)
                        const result = await repairAuthServerSignInForUser(user.id, next, readCurrentUser())
                        setRepairBusy(false)
                    ...
                    onClick={() => {
                      void (async () => {
                        const next = await appPrompt(
                          ...
                          '',
                          { title: 'Reset password', confirmLabel: 'Apply' },
                        )
                        if (!next) return
                        setResetBusy(true)
                        const result = await ownerResetAccountPassword(user.id, next, readCurrentUser())
                        setResetBusy(false)
                        onPasswordReset?.(result.message)
                        if (result.ok) {
                          onPatch({
