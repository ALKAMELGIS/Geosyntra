...
export interface LoginCanvasGlobeProps {
  className?: string
  /** Max canvas edge in CSS pixels when not filling parent */
  size?: number
  dotColor?: string
  arcColor?: string
  /** Must end with `, 1)` for pulse stroke alpha replacement */
  markerColor?: string
...
const DEFAULT_MARKERS: { lat: number; lng: number }[] = [
...
export function LoginCanvasGlobe({
  className,
  size = 560,
  ...
  return (
    <canvas
      ref={canvasRef}
      className={cn('login-canvas-globe block w-full h-full cursor-grab touch-none active:cursor-grabbing', className)}
      style={{ maxWidth: size, maxHeight: size, width: '100%', height: '100%' }}
