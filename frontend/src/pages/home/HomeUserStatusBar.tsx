...
function GuestAuthMenu({ onSignIn, onRegister }: { onSignIn: () => void; onRegister: () => void }) {
  return (
    <div className="home-nav-auth__guest-popover" role="menu" aria-label="Sign in or register">
      <ul className="home-nav-auth__guest-list">
        <li>
          <button type="button" role="menuitem" className="home-nav-auth__guest-action" onClick={onSignIn}>
            Sign in
          </button>
        </li>
        <li>
          <button type="button" role="menuitem" className="home-nav-auth__guest-action" onClick={onRegister}>
            Register
          </button>
        </li>
      </ul>
...
function NavAuthTrigger({
  ...
  return (
    ...
    >
      ...
      {!isGuest ? <i className="fa-solid fa-chevron-down home-nav-auth__chevron" aria-hidden /> : null}
    </button>
  )
}
...
export function HomeUserStatusBar() {
  ...
  const onHome = location.pathname === SAAS_ROUTES.home
  ...
  const openSignUp = () => {
    setMenuOpen(false)
    if (onHome) {
      openWizard({ step: 'welcome', authMode: 'signup' })
      return
    }
    navigateToHomeWizard(navigate, { wizard: 'auth', authMode: 'signup' })
  }
  ...
  if (!user) {
    return (
      <div ref={rootRef} className="home-user-toolbar home-user-toolbar--nav-auth home-user-toolbar--nav-auth-guest">
        ...
        {menuOpen ? (
          <div className="home-user-toolbar__dropdown home-user-toolbar__dropdown--guest">
            <GuestAuthMenu onSignIn={openSignIn} onRegister={openSignUp} />
          </div>
        ) : null}
      </div>
    )
