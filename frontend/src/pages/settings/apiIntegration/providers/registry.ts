...
const PROVIDERS: ProviderConfig[] = [
  ...
  },
  {
    ...
    description: 'Basemaps and geocoding for Satellite Intelligence — MAPBOX_TOKEN on the server only.',
    capabilities: ['Basemaps', 'Geocoding', 'Vector tiles'],
    defaultBaseUrl: 'https://api.mapbox.com',
    authTypes: ['api_key'],
    defaultAuthType: 'api_key',
    envOnly: true,
    fieldsByAuth: { api_key: [] },
  },
  {
    id: 'google_maps',
    label: 'Google Maps',
  ...
  },
  {
    ...
    description: 'Open-source routing for Geo AI — driving, walking, cycling, alternatives.',
    capabilities: ['Directions', 'Isochrones', 'Matrix'],
    defaultBaseUrl: 'https://api.openrouteservice.org',
    authTypes: ['api_key'],
    defaultAuthType: 'api_key',
    vaultTypeId: 'orsApiKey',
    fieldsByAuth: {
      api_key: apiKeyOnly('ORS API Key (OPENROUTESERVICE_API_KEY)', 'apiKey'),
    },
  },
  {
    id: 'graphhopper',
    label: 'GraphHopper',
    category: 'gis',
    iconClass: 'fa-solid fa-map-signs',
    description:
      'Street routing, navigation, isochrones, and Route Map tool — inspired by GraphHopper Maps open-source UI.',
    capabilities: ['Directions', 'Navigation', 'Isochrones', 'Route optimization', 'Geocoding'],
    defaultBaseUrl: 'https://graphhopper.com/api/1',
    authTypes: ['api_key'],
    defaultAuthType: 'api_key',
    vaultTypeId: 'graphHopperApiKey',
    fieldsByAuth: {
      api_key: fields(
        field('apiKey', 'GraphHopper API Key', {
          required: true,
          secret: SECRET,
          kind: 'password',
          hint: 'graphhopper.com → Dashboard → API keys. Powers Route Map on Satellite Intelligence.',
        }),
      ),
    },
  },
...
export function getProvider(id: ProviderId): ProviderConfig {
  return PROVIDER_REGISTRY[id] ?? PROVIDER_REGISTRY.arcgis_online
}
...
export function providerFromLegacyTypeId(typeId: string): ProviderId {
  if (typeId === 'mapboxToken' || typeId === 'mapboxIntegration') return 'mapbox'
  ...
  return map[typeId] ?? 'arcgis_online'
}
