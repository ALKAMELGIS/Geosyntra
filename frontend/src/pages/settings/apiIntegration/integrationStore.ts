...
import { scheduleApiVaultCatalogSync } from '../../../lib/apiVaultPersistence'
...
export function saveIntegrationRecord(
  ...
  const config = provider.envOnly ? { ...draft.config } : { ...draft.config, ...secrets }
  const typeId = (
    provider.envOnly
      ? 'mapboxIntegration'
      : (provider.vaultTypeId ?? 'arcgisPortalToken')
  ) as ApiTokenTypeId
  const input = {
    name: draft.name,
    typeId,
...
export function emptyDraft(providerId: ProviderId = 'arcgis_online'): IntegrationDraft {
