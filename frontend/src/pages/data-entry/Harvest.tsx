...
type WorkflowFormKey = 'Harvest' | 'Production'
type SaveRecord = {
  id: string
  tsUtc: string
...
export default function Harvest() {
  ...
  const isProductionPage = location.pathname.startsWith('/data/production')
  const formKey: WorkflowFormKey = isProductionPage ? 'Production' : 'Harvest'
  const workflowSlug = isProductionPage ? 'production' : 'harvest'
  const shell = useMemo(() => getWorkflowShellMeta(workflowSlug, uiLang), [workflowSlug, uiLang])
  const steps = useMemo(() => stepLabels(uiLang), [uiLang])
  const storageKey = isProductionPage ? 'production_tracking_records_v1' : 'harvest_logging_records_v1'
  ...
  const persistLocal = (kind: 'draft' | 'submitted') => {
    try {
      ...
      const existing = readJson<SaveRecord[]>(storageKey, [])
  ...
  const recipesPath = formKey === 'Production' ? '/data/recipes/production' : '/data/recipes/harvest'
  ...
  return (
    <div className="ec-page workflow-shell-page">
      <div className="ec-container ec-animate-in">
        <WorkflowHeroStepper
          ...
          dataEntryPath={isProductionPage ? '/data/production' : '/data/harvest'}
        ...
        <div className="ec-card">
          <div className="ec-card-header">
            ...
            <div className="ec-card-header-actions">
              {canManageSettings ? (
                <button type="button" className="ec-icon-btn" aria-label="Open workflow settings" title="Settings" onClick={() => attemptNavigate(`/master/workflow-settings?form=${formKey}`)}>
