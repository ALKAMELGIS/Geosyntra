...
export default function PrimaryNavIcons({ onLogout }: PrimaryNavIconsProps) {
  ...
  return (
    <nav
      ref={rootRef}
      className={`geosyntra-primary-nav${isMobile ? ' geosyntra-primary-nav--mobile' : ''}`}
          ...
          return (
            <div key={group.id} className="geosyntra-primary-nav__slot">
              <NavLink
                to={unifiedPath}
                className={({ isActive }) =>
                  'geosyntra-primary-nav__trigger geosyntra-primary-nav__trigger--link' +
                  (isActive || active ? ' geosyntra-primary-nav__trigger--active' : '')
                }
