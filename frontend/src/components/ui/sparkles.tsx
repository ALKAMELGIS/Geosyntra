 ...
 * the 21st.dev Sparkles brief.
 *
 * Renders a tsparticles canvas that drifts soft white dots upward — used in
 * the Geosyntra landing hero as the "starfield bar" beneath the brand mark
 * (mirrors the reference Acme example, but with the Geosyntra wordmark
 * sitting above instead of Acme).
 *
 * The component is lazy-init: tsparticles only boots once `loadSlim` has
 * resolved (`init` flag), and the canvas fades in via a 1 s framer-motion
 * tween so the field never pops in. All knobs (`minSize`, `maxSize`,
 * `speed`, `particleColor`, `particleDensity`) match the upstream prop
 * surface so existing demos drop in unchanged.
 */
type ParticlesProps = {
  id?: string
  className?: string
