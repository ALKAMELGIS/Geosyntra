...
import './navmenu.css'
import { hasPermission, isPlatformOwnerUser, normalizeRole, readCurrentUser } from '../lib/auth'
import { useLanguage } from '../lib/i18n'
import type { MergedGroup } from '../nav/navManifest'
import { singleVisibleNavPath } from '../nav/navManifest'
...
export default function NavMenu({ onLogout }: NavMenuProps) {
  ...
  const isOwner = isPlatformOwnerUser(currentUser)
  ...
  const renderMergedGroup = (group: MergedGroup) => {
    ...
    const unifiedPath = singleVisibleNavPath(children)
    if (unifiedPath) {
      const leaf = children[0]!
      return (
        <li key={group.id} className="navmenu-li">
          <NavLink
            to={unifiedPath}
            title={navLabel(group)}
            aria-label={navLabel(group)}
            onClick={handleNavigate}
            onMouseEnter={() => prefetchRoute(unifiedPath)}
            onPointerEnter={() => prefetchRoute(unifiedPath)}
            onFocus={() => prefetchRoute(unifiedPath)}
            className={({ isActive }) =>
              isActive
                ? `item active ${group.headerClass} ${leaf.subitemClass}`
                : `item ${group.headerClass} ${leaf.subitemClass}`
            }
          >
            <span className="icon">
  ...
  useEffect(() => {
    ...
    document.body.classList.remove('nav-drawer-viewport', 'nav-drawer-open')
  }, [viewport])
  ...
  // Phone: compact icon rail lives in HeaderBar (PrimaryNavIcons).
  if (viewport === 'mobile') {
    return null
  }
