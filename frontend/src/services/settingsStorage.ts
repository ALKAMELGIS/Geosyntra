...
export function mergeWithDefaults(partial: Partial<SystemSettingsPersistedV1>): SystemSettingsPersistedV1 {
  ...
  return {
    ...
    headerSettings: {
      ...
      fontSize: (() => {
        const raw = Number(hdrRaw?.fontSize ?? DEFAULT_SYSTEM_SETTINGS.headerSettings.fontSize)
          || DEFAULT_SYSTEM_SETTINGS.headerSettings.fontSize
        // Migrate legacy compact defaults to the current lux wordmark size (22).
        if (raw === 15 || raw === 17) return DEFAULT_SYSTEM_SETTINGS.headerSettings.fontSize
        return Math.max(10, Math.min(42, raw))
      })(),
      // Migrate legacy light/bold defaults to the current lux wordmark weight (600).
      fontWeight: (() => {
        ...
        if (raw === 700 || raw === 300) return DEFAULT_SYSTEM_SETTINGS.headerSettings.fontWeight
...
function sanitizeCustomPage(raw: unknown): CustomPageRecord | null {
  ...
  const rawBind = String(r.bindTarget)
  const bindTarget = (
    rawBind === 'gis'
      ? 'satellite-indices'
      : ['placeholder', 'home', 'satellite-indices', 'dashboards-overview'].includes(rawBind)
        ? rawBind
        : 'placeholder'
