/**
 * GitHub Pages / custom domain over plain HTTP: browsers hide crypto.subtle.
 * Load this synchronously before the SPA module so legacy bundles can call digest('SHA-256').
 */
;(function (g) {
  'use strict'
  if (typeof g === 'undefined') return
  if (!g.crypto) g.crypto = {}
  if (g.crypto.subtle && typeof g.crypto.subtle.digest === 'function') return

  function rotr(x, n) {
    return (x >>> n) | (x << (32 - n))
  }

  function sha256(u8) {
    const K = new Uint32Array([
      0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
      0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
      0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
      0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
      0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
      0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
      0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
      0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
    ])
    const bitLen = u8.length * 8
    const padLen = ((56 - ((u8.length + 1) % 64)) + 64) % 64
    const padded = new Uint8Array(u8.length + 1 + padLen + 8)
    padded.set(u8)
    padded[u8.length] = 0x80
    const dv = new DataView(padded.buffer)
    dv.setUint32(padded.length - 4, Math.floor(bitLen / 0x100000000), false)
    dv.setUint32(padded.length - 8, bitLen >>> 0, false)

    let h0 = 0x6a09e667
    let h1 = 0xbb67ae85
    let h2 = 0x3c6ef372
    let h3 = 0xa54ff53a
    let h4 = 0x510e527f
    let h5 = 0x9b05688c
    let h6 = 0x1f83d9ab
    let h7 = 0x5be0cd19
    const w = new Uint32Array(64)

    for (let off = 0; off < padded.length; off += 64) {
      for (let i = 0; i < 16; i++) w[i] = dv.getUint32(off + i * 4, false)
      for (let i = 16; i < 64; i++) {
        const s0 = rotr(w[i - 15], 7) ^ rotr(w[i - 15], 18) ^ (w[i - 15] >>> 3)
        const s1 = rotr(w[i - 2], 17) ^ rotr(w[i - 2], 19) ^ (w[i - 2] >>> 10)
        w[i] = (w[i - 16] + s0 + w[i - 7] + s1) >>> 0
      }
      let a = h0
      let b = h1
      let c = h2
      let d = h3
      let e = h4
      let f = h5
      let gb = h6
      let h = h7
      for (let i = 0; i < 64; i++) {
        const S1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25)
        const ch = (e & f) ^ (~e & gb)
        const t1 = (h + S1 + ch + K[i] + w[i]) >>> 0
        const S0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22)
        const maj = (a & b) ^ (a & c) ^ (b & c)
        const t2 = (S0 + maj) >>> 0
        h = gb
        gb = f
        f = e
        e = (d + t1) >>> 0
        d = c
        c = b
        b = a
        a = (t1 + t2) >>> 0
      }
      h0 = (h0 + a) >>> 0
      h1 = (h1 + b) >>> 0
      h2 = (h2 + c) >>> 0
      h3 = (h3 + d) >>> 0
      h4 = (h4 + e) >>> 0
      h5 = (h5 + f) >>> 0
      h6 = (h6 + gb) >>> 0
      h7 = (h7 + h) >>> 0
    }
    const out = new Uint8Array(32)
    const outDv = new DataView(out.buffer)
    outDv.setUint32(0, h0, false)
    outDv.setUint32(4, h1, false)
    outDv.setUint32(8, h2, false)
    outDv.setUint32(12, h3, false)
    outDv.setUint32(16, h4, false)
    outDv.setUint32(20, h5, false)
    outDv.setUint32(24, h6, false)
    outDv.setUint32(28, h7, false)
    return out
  }

  const subtle = g.crypto.subtle || {}
  subtle.digest = function (algorithm, data) {
    const name =
      typeof algorithm === 'string' ? algorithm : algorithm && algorithm.name ? algorithm.name : ''
    if (name !== 'SHA-256') {
      return Promise.reject(new DOMException('Unsupported algorithm', 'NotSupportedError'))
    }
    const bytes =
      data instanceof ArrayBuffer
        ? new Uint8Array(data)
        : ArrayBuffer.isView(data)
          ? new Uint8Array(data.buffer, data.byteOffset, data.byteLength)
          : new Uint8Array(0)
    return Promise.resolve(sha256(bytes).buffer)
  }
  g.crypto.subtle = subtle
})(typeof globalThis !== 'undefined' ? globalThis : typeof window !== 'undefined' ? window : this)
