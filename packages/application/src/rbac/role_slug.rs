//! Express RBAC role slug helpers for application use cases (mirrors infrastructure `role_slug`).
...
/// Express `roleRank` — higher rank = more privilege.
pub fn role_rank(slug: &str) -> u32 {
    match normalize_rbac_role(slug) {
        "trial_user" => 6,
        "viewer" | "user" => 14,
        "ai_operator" => 18,
