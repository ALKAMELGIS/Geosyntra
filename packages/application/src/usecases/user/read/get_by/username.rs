...
pub struct GetUserByUsernameUseCase {
    repo: Arc<dyn UserRepository>,
    auth: Arc<dyn AuthorizationService>,
}

impl GetUserByUsernameUseCase {
    ...
    pub async fn execute(
        ...
        if let Some(id) = self
            .repo
            .find_user_id_by_username(ctx.clone(), username.clone())
            .await?
            && id == *ctx.user_id()
        {
        ...
        let view = self.repo.fetch_view_by_username(ctx, username, &access).await?;
        Ok(UserProjector::present(view, &access))
    }
}

impl UseCaseDescriptor for GetUserByUsernameUseCase {
